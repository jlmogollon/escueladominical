...
function getDb(){
  ...
  try{
    ...
    if(!fb)return null;  // SDK no cargado aÃºn â€” waitForDb reintentarÃ¡
    if(!fb.apps||!fb.apps.length)fb.initializeApp(FIREBASE_CONFIG);
    const db=fb.firestore?fb.firestore():null;
    if(db){_dbInstance=db;console.log("[Firebase] Firestore conectado OK âœ“");}
    return db;
  }catch(e){
...
// Espera hasta que Firebase estÃ© listo (mÃ¡x 15s, reintento cada 300ms)
...
function firebaseDiagnostico(){
  waitForDb(5000).then(d=>{
    console.log("[Firebase] DiagnÃ³stico â€” firebase:",!!window.firebase,"| db:",d?"OK":"null");
    if(!d){console.warn("[Firebase] No hay conexiÃ³n. Verifica que los scripts de Firebase se carguen.");return;}
    d.collection(FIRESTORE_COLLECTION).doc("_test_connection")
      .set({value:JSON.stringify({t:Date.now()})},{merge:true})
      .then(()=>console.log("[Firebase] Escritura de prueba OK âœ“"))
...
// Solo nube: siempre lee desde el servidor para sincronizaciÃ³n en tiempo real.
async function loadData(key){
  ...
  }catch(e){ console.error("loadData fallÃ³ ["+key+"]:",e&&e.message?e.message:e); return null; }
}
...
async function saveData(key,val){
  try{
    ...
    console.error("saveData fallÃ³ ["+key+"]:",e&&e.message?e.message:e);
    queuePendingWrite(key,val);
    return false;
  }
}
...
// Muestra solo primer nombre + primer apellido. Si el registro tiene las 4 partes guardadas, las usa (asÃ­ no se "ruedan" al editar).
...
// Caso especial: algunos maestros de ADOLESCENTES estÃ¡n almacenados como "Apellido Nombre(s)".
...
// Parsea para el formulario asumiendo "Apellido Nombre(s)": asÃ­ al guardar con buildNombreFull queda "Nombre(s) Apellido(s)" y se unifica la base.
...
// Si la calificaciÃ³n total es la mÃ¡xima (5), mostrar "5" sin decimales.
function formatEvalScore(avg){
  if(avg==null||avg==="â€”")return avg;
  const n=parseFloat(avg);
  if(Number.isNaN(n))return avg;
  if(n>=5)return "5";
  return String(avg);
}
// Calcula score de video 0-5 por sesiÃ³n
...
// Promedio de videos de un maestro (0-5). Maestros de ADOLESCENTES no tienen evaluaciÃ³n de videos.
...
// SesiÃ³n con fecha al menos 7 dÃ­as en el pasado (para evaluar cumplimiento)
...
// Cumplimiento de clases (1-5): segÃºn clases asignadas. Empieza en 5; cada falla registrada resta 1 punto (mÃ­nimo 1). Con una falla nunca puede tener 5 estrellas. Solo se calcula si tiene al menos una sesiÃ³n como maestro/auxiliar o una falla.
...
// â”€â”€ Logo with elegant gradient border & edge fade. Si onClick, al tocar hace recarga en limpio. â”€â”€
...
function AvatarUpload({ photo, onPhoto, size=56, initials="?", color="#5B2D8E" }) {
  ...
  return (
    <div style={{ position:"relative", width:size, height:size, flexShrink:0 }}>
      ...
      <button onClick={(e)=>{e.stopPropagation();ref.current.click();}} style={{ position:"absolute", bottom:-2, right:-2, width:22, height:22, borderRadius:"50%", background:color, border:"2px solid #FFFFFF", cursor:"pointer", fontSize:11, color:"#FFFFFF", display:"flex", alignItems:"center", justifyContent:"center", padding:0 }}>ðŸ“·</button>
    ...
    <div data-modal-overlay style={{position:"fixed",inset:0,background:"rgba(30,10,60,0.65)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)",overflow:"hidden",overscrollBehavior:"contain"}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div data-modal-content style={{background:"#FFFFFF",borderRadius:"24px 24px 0 0",padding:"1rem 1rem 1.5rem",width:"100%",maxWidth:520,maxHeight:"92vh",overflowY:"auto",overflowX:"hidden",boxShadow:"0 -8px 40px rgba(0,0,0,0.25)",minWidth:0,WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1rem",minWidth:0}}>
          ...
          <button onClick={onClose} style={{background:"#F5F0FF",border:"none",borderRadius:"50%",width:32,height:32,fontSize:18,cursor:"pointer",flexShrink:0}}>Ã—</button>
        </div>
...
// Un solo banner: los 2 cumpleaÃ±os mÃ¡s cercanos (alumno o maestro/auxiliar)
      ...
      {dosMasCercanos.map((b,i)=>(
        b.diff===0 ? (
          <div key={i} style={{background:"linear-gradient(135deg,#E84F9B,#F27DB8)",borderRadius:16,padding:"14px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:i<dosMasCercanos.length-1?8:0,boxShadow:"0 4px 16px rgba(232,79,155,0.3)"}}>
            ...
            <div style={{flex:1}}><div style={{color:"#FFFFFF",fontWeight:900,fontSize:16}}>Â¡CumpleaÃ±os hoy!</div><div style={{color:"rgba(255,255,255,0.9)",fontWeight:700,fontSize:14}}>{b.nombre}</div><div style={{color:"rgba(255,255,255,0.75)",fontSize:12}}>{b.tipo}{b.clase&&b.categoria==="alumno"?" Â· "+b.clase:""}</div></div>
          ...
          <div key={i} style={{background:"linear-gradient(135deg,#5B2D8E,#7B4DB2)",borderRadius:16,padding:"0.75rem 1rem",display:"flex",alignItems:"center",gap:12,marginBottom:i<dosMasCercanos.length-1?8:0,boxShadow:"0 4px 16px rgba(91,45,142,0.2)"}}>
            ...
            <div style={{flex:1}}><div style={{color:"#F5C842",fontWeight:800,fontSize:12}}>PrÃ³ximo cumpleaÃ±os</div><div style={{color:"#FFFFFF",fontWeight:700,fontSize:14}}>{b.nombre}</div><div style={{color:"rgba(255,255,255,0.7)",fontSize:12}}>{b.tipo}{b.clase&&b.categoria==="alumno"?" Â· "+b.clase:""} Â· {b.fecha}</div></div>
...
// VersÃ­culo bÃ­blico diario â€” cambia cada dÃ­a (Ã­ndice por dÃ­a del aÃ±o)
const VERSICULOS_DIARIOS = [
  { ref: "Proverbios 22:6", texto: "Instruye al niÃ±o en su camino, Y aun cuando fuere viejo no se apartarÃ¡ de Ã©l.", url: "https://www.bible.com/bible/149/PRO.22.6" },
  { ref: "Deuteronomio 6:7", texto: "Y las repetirÃ¡s a tus hijos, y hablarÃ¡s de ellas estando en tu casa, y andando por el camino.", url: "https://www.bible.com/bible/149/DEU.6.7" },
  { ref: "Salmos 127:3", texto: "Herencia de JehovÃ¡ son los hijos; cosa de estima el fruto del vientre.", url: "https://www.bible.com/bible/149/PSA.127.3" },
  { ref: "Mateo 19:14", texto: "Dejad a los niÃ±os venir a mÃ­, y no se lo impidÃ¡is; porque de los tales es el reino de los cielos.", url: "https://www.bible.com/bible/149/MAT.19.14" },
  { ref: "IsaÃ­as 54:13", texto: "Y todos tus hijos serÃ¡n enseÃ±ados por JehovÃ¡; y se multiplicarÃ¡ la paz de tus hijos.", url: "https://www.bible.com/bible/149/ISA.54.13" },
  { ref: "2 Timoteo 3:15", texto: "Y que desde la niÃ±ez has sabido las Sagradas Escrituras, las cuales te pueden hacer sabio para la salvaciÃ³n.", url: "https://www.bible.com/bible/149/2TI.3.15" },
  { ref: "Salmos 78:4", texto: "No las encubriremos a sus hijos, contando a la generaciÃ³n venidera las alabanzas de JehovÃ¡.", url: "https://www.bible.com/bible/149/PSA.78.4" },
  { ref: "Efesios 6:4", texto: "Y vosotros, padres, no provoquÃ©is a ira a vuestros hijos, sino criadlos en disciplina y amonestaciÃ³n del SeÃ±or.", url: "https://www.bible.com/bible/149/EPH.6.4" },
  { ref: "Colosenses 3:21", texto: "Padres, no exasperÃ©is a vuestros hijos, para que no se desalienten.", url: "https://www.bible.com/bible/149/COL.3.21" },
  { ref: "Proverbios 29:17", texto: "Corrige a tu hijo, y te darÃ¡ descanso, y darÃ¡ alegrÃ­a a tu alma.", url: "https://www.bible.com/bible/149/PRO.29.17" },
];
  ...
  return(
    <a href={url} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none",display:"block",margin:"0 16px 14px",minHeight:72}}>
      <div style={{background:"linear-gradient(135deg,#E8F5E9,#C8E6C9)",borderRadius:16,padding:"14px 16px",boxShadow:"0 2px 12px rgba(129,199,132,0.2)",borderLeft:"5px solid #81C784",cursor:"pointer",transition:"box-shadow 0.2s",minHeight:72,boxSizing:"border-box"}} onMouseOver={e=>{e.currentTarget.style.boxShadow="0 4px 16px rgba(129,199,132,0.3)";}} onMouseOut={e=>{e.currentTarget.style.boxShadow="0 2px 12px rgba(129,199,132,0.2)";}}>
        <div style={{color:"#2E7D32",fontSize:14,lineHeight:1.5,fontStyle:"italic",marginBottom:6}}>Â«{texto}Â»</div>
        <div style={{color:"#558B2F",fontSize:12,fontWeight:700}}>ðŸ“– {ref}</div>
      </div>
    </a>
  );
}
...
// â•â•â•â•â•â•â•â•â•â• ERROR BOUNDARY â•â•â•â•â•â•â•â•â•â•
