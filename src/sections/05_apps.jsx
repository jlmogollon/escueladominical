...
function TeacherApp({user,data,onLogout,onUpdateData,teacherPasswords,onUpdatePasswords,onRefreshData}){
  ...
  const saveEditNino=()=>{
    ...
    // Auto-derive cumpleaÃ±os and edad from nacimiento
    if(form.nacimiento){
      try{
  ...
  const changePw=async()=>{
    ...
    if(pwForm.old!==cur){setPwError("ContraseÃ±a actual incorrecta");return;}
    if(pwForm.new1.length<4){setPwError("MÃ­nimo 4 caracteres");return;}
    ...
    else setPwError("No se pudo guardar en la nube. Revisa la conexiÃ³n o la consola (F12).");
  };
  const handlePhoto=async(e)=>{const file=e.target.files[0];if(!file)return;const compressed=await compressImage(file,128,0.5);setPhotoSrc(compressed);const updated=maestros.map(m=>m.nombre===user.name?{...m,foto:compressed}:m);const ok=await onUpdateData("maestros",updated);if(!ok)alert("No se pudo guardar la foto en la nube. Revisa la conexiÃ³n o la consola (F12).");};
  ...
  const deletePeticionT=()=>{if(peticionEditIdT&&confirmDelete("Â¿Eliminar esta peticiÃ³n de oraciÃ³n?")){onUpdateData("peticiones",peticiones.filter(p=>p.id!==peticionEditIdT));setPeticionModalT(false);}};

  const tabs=[
    {id:"inicio",label:"Inicio",icon:"ðŸ "},
    {id:"cronograma",label:"Horario",icon:"ðŸ“…"},
    {id:"calificaciones",label:"Calific.",icon:"ðŸ“Š"},
    {id:"clase",label:"Mi clase",icon:"ðŸ‘§"},
    {id:"clases",label:"Clases",icon:"ðŸ§’"},
    {id:"mas",label:"MÃ¡s",icon:"â˜°"},
  ];
      ...
      <div style={{background:"linear-gradient(135deg,#3D1B6B,#5B2D8E)",padding:"0.75rem 1rem",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 2px 12px rgba(0,0,0,0.15)",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{textAlign:"right"}}>
            ...
            <div style={{color:"rgba(255,255,255,0.65)",fontSize:10}}>{teacherInfo.cargo} Â· {miClase}</div>
          </div>
          <button style={{background:"rgba(255,255,255,0.18)",border:"none",borderRadius:10,padding:"8px 12px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}} onClick={onLogout}>ðŸšª Salir</button>
        </div>
      </div>
      ...
      <div style={{paddingBottom:10}}>
        {activeTab==="inicio"&&(
          <div style={{padding:"1rem 1rem 0"}}>
            <h2 style={S.title}>Hola, {displayMaestroNombre(user.name).split(" ")[0]} ðŸ‘‹</h2>
            <BirthdayBanner maestros={maestros} familias={familias}/>
            <VerseBannerMaestros/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              <StatCard icon="ðŸ‘§" value={misNinos.length} label="Mis alumnos" color={CLASE_COLORS[miClase]||"#5B2D8E"} onClick={()=>setActiveTab("clase")}/>
              <StatCard icon="ðŸ“…" value={misClases.length} label="Clases asig." color="#4BBCE0" onClick={()=>setActiveTab("cronograma")}/>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            ...
            <div style={S.card}>
              <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,marginBottom:12}}>ðŸ“… PrÃ³ximas Clases</h3>
              {misClases.slice(0,3).map((c,i)=>(
                <div key={i} style={{marginBottom:10,background:(CLASE_COLORS[c.grupo]||"#5B2D8E")+"15",border:`1.5px solid ${CLASE_COLORS[c.grupo]||"#5B2D8E"}44`,borderRadius:12,padding:"12px 14px"}}>
                  <div style={{fontWeight:800,color:CLASE_COLORS[c.grupo]||"#5B2D8E",fontSize:12}}>{formatFecha(c.fecha)} Â· {c.grupo}</div>
                  <div style={{fontSize:15,fontWeight:700,margin:"4px 0 2px"}}>{c.leccion}</div>
                  <div style={{fontSize:12,color:"#7B6B9A"}}>{c.tema}</div>
                  <div style={{marginTop:6}}><span style={S.badge(sameTeacherName(c.maestro,user.name)?"#5B2D8E":"#4BBCE0")}>{sameTeacherName(c.maestro,user.name)?"ðŸŽ“ Maestro":"ðŸ¤ Auxiliar"}</span></div>
                </div>
              ))}
            ...
            <div style={S.card}>
              ...
              {upcomingEventsTeacher.length===0&&<div style={{color:"#7B6B9A"}}>Sin eventos prÃ³ximos.</div>}
              ...
              {sortedPeticionesT.map(p=>(
                <div key={p.id} style={{padding:"10px 0",borderBottom:"1px solid #DDD0F0"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6,marginTop:6}}>
                    ...
                    {p.autor===user.name&&<button style={{...S.btn("#4BBCE0"),padding:"5px 10px",fontSize:12}} onClick={()=>openEditPeticionT(p)}>âœï¸</button>}
                  </div>
                </div>
              ))}
              <div style={{marginTop:16,paddingTop:16,borderTop:"2px solid #E84F9B33"}}>
                <div style={{fontSize:12,color:"#7B6B9A",marginBottom:8}}>AÃ±adir peticiÃ³n (visible para todos)</div>
                <textarea style={{...S.input,height:80,resize:"vertical",marginBottom:10}} value={peticionFormT.texto} onChange={e=>setPeticionFormT(f=>({...f,texto:e.target.value}))} placeholder="Escribe tu peticiÃ³n de oraciÃ³n..."/>
                ...
                <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:12,fontSize:14}} onClick={savePeticionNuevaT}>ðŸ™ Guardar PeticiÃ³n</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{padding:"1rem 1rem 0"}}>
            ...
            {misClases.map((c,i)=>(
              <div key={i} style={{...S.card,borderLeft:`5px solid ${CLASE_COLORS[c.grupo]||"#5B2D8E"}`}}>
                <div style={{fontWeight:800,color:CLASE_COLORS[c.grupo]||"#5B2D8E",fontSize:13}}>{formatFecha(c.fecha)} 2026 Â· {c.grupo}</div>
                ...
                <div style={{display:"flex",gap:10}}>
                  <span style={S.badge(sameTeacherName(c.maestro,user.name)?"#5B2D8E":"#4BBCE0")}>{sameTeacherName(c.maestro,user.name)?"ðŸŽ“ Maestro":"ðŸ¤ Auxiliar"}</span>
                  <span style={{fontSize:12,color:"#7B6B9A"}}>Aux: {c.auxiliar?displayMaestroNombre(c.auxiliar):"â€”"}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab==="clase"&&(
          <div style={{padding:"1rem 1rem 0"}}>
            ...
            {misNinos.map((n,i)=>{
              ...
              // Padres/telÃ©fonos solo desde la tarjeta del alumno (n), nunca desde familias u otra fuente
              ...
              return(
                <div key={n.id||i} style={{...S.card,borderLeft:`5px solid ${CLASE_COLORS[miClase]||"#5B2D8E"}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:12,color:"#7B6B9A"}}>
                        {n.edad?n.edad+" aÃ±os Â· ":""}
                        {totalSesiones} sesiones asistidas
                        {p?.bautizado&&" Â· ðŸ™ Bautizado"}
                        {p?.sellado&&" Â· âœ¨ Sellado"}
                      </div>
                    </div>
                      ...
                      {obsHistorial.map((obs,oi)=>(
                        <div key={obs.id||oi} style={{marginBottom:oi<obsHistorial.length-1?10:0,paddingBottom:oi<obsHistorial.length-1?10:0,borderBottom:oi<obsHistorial.length-1?"1px dashed #F5C84255":"none"}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                            <span style={{fontSize:11,fontWeight:800,color:"#5B2D8E",background:"#5B2D8E15",borderRadius:8,padding:"2px 7px"}}>{obs.leccion||"SesiÃ³n"}</span>
                  ...
                  {obsHistorial.length===0&&totalSesiones>0&&(
                    <div style={{fontSize:12,color:"#7B6B9A",fontStyle:"italic",padding:"4px 0 8px",borderBottom:"1px solid #F0ECF8",marginBottom:8}}>Sin observaciones registradas aÃºn</div>
                  )}
                  {p&&(p.padre||p.madre)&&(
                    <div style={{background:"#F5F0FF",borderRadius:12,padding:"10px 14px"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                        ...
                        {!alumnosSource&&<button style={{...S.btn("#F5C842","#3D1B6B"),padding:"4px 10px",fontSize:11}} onClick={()=>openPhoneEdit(p)}>âœï¸ Editar Tel.</button>}
                      </div>
                      {p.padre&&p.padre!=="(No registra)"&&(
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:p.madre?8:0}}>
                          ...
                          {p.telPadre?<a href={`tel:${p.telPadre}`} style={{background:"#4BBCE0",color:"#FFFFFF",borderRadius:10,padding:"6px 12px",fontSize:12,fontWeight:800,textDecoration:"none",whiteSpace:"nowrap"}}>ðŸ“ž {p.telPadre}</a>:<span style={{fontSize:11,color:"#EF5350",fontStyle:"italic"}}>Sin tel.</span>}
                        </div>
                      )}
                      {p.madre&&p.madre!=="(No registra)"&&(
                        <div style={{display:"flex",alignItems:"center",gap:8}}>
                          ...
                          {p.telMadre?<a href={`tel:${p.telMadre}`} style={{background:"#E84F9B",color:"#FFFFFF",borderRadius:10,padding:"6px 12px",fontSize:12,fontWeight:800,textDecoration:"none",whiteSpace:"nowrap"}}>ðŸ“ž {p.telMadre}</a>:<span style={{fontSize:11,color:"#EF5350",fontStyle:"italic"}}>Sin tel.</span>}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {p&&!(p.padre&&p.padre!=="(No registra)")&&!(p.madre&&p.madre!=="(No registra)")&&!alumnosSource&&(
                    <div style={{background:"#FFF0E6",borderRadius:10,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
                      <div style={{flex:1,fontSize:12,color:"#7B6B9A"}}>âš ï¸ Padres sin nombre registrado</div>
                      <button style={{...S.btn("#F5C842","#3D1B6B"),padding:"6px 12px",fontSize:12}} onClick={()=>openPhoneEdit(p)}>âœï¸ Editar</button>
                    </div>
                  )}
                </div>
              );
        ...
        <Modal open={phoneModal} onClose={()=>setPhoneModal(false)} title="Editar TelÃ©fonos de Padres">
          <div style={{background:"#F5F0FF",borderRadius:12,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#7B6B9A"}}>Agrega o actualiza los telÃ©fonos de contacto</div>
          {[["ðŸ‘¨ Padre / Tutor","padre"],["ðŸ“ž Tel. Padre","telPadre"],["ðŸ‘© Madre / Tutora","madre"],["ðŸ“ž Tel. Madre","telMadre"]].map(([l,k])=>(
          ...
          <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:14}} onClick={savePhone}>ðŸ’¾ Guardar TelÃ©fonos</button>
        </Modal>
        <Modal open={editNinoModal} onClose={()=>setEditNinoModal(false)} title="âœï¸ Editar Alumno">
          <div style={{background:"#F0FBFF",borderRadius:12,padding:"10px 14px",marginBottom:14,fontSize:13,color:"#2A96BC",lineHeight:1.6}}>El cumpleaÃ±os y la edad se calculan automÃ¡ticamente a partir de la fecha de nacimiento.</div>
          ...
          {editNinoForm.nacimiento&&(()=>{
            try{
              ...
              return <div style={{background:"#F5FFF5",border:"2px solid #4CAF5044",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
                <div style={{fontSize:12,color:"#7B6B9A",marginBottom:4}}>Calculado automÃ¡ticamente:</div>
                <div style={{display:"flex",gap:16}}>
                  <div><span style={{fontSize:11,color:"#7B6B9A"}}>ðŸŽ‚ CumpleaÃ±os</span><div style={{fontWeight:800,color:"#4CAF50"}}>{cumple}</div></div>
                  <div><span style={{fontSize:11,color:"#7B6B9A"}}>ðŸ”¢ Edad</span><div style={{fontWeight:800,color:"#4CAF50"}}>{edad} aÃ±os</div></div>
                </div>
              </div>;
            }catch(e){return null;}
          })()}
          <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:14}} onClick={saveEditNino}>ðŸ’¾ Guardar Cambios</button>
        </Modal>
        {activeTab==="mas"&&(
          <div style={{padding:"1rem 1rem 0"}}>
            <div style={{display:"flex",gap:8,overflowX:"auto",marginBottom:16,paddingBottom:4}}>
              {[
                ["eventos","ðŸ“† Eventos"],
                ["cumpleanos","ðŸŽ‚ Cumple."],
                ["finanzas","ðŸ’° Finanzas"],
                ...((()=>{const n=(user.name||"").toLowerCase();return n.includes("marcela")&&n.includes("lavaire")?[["finanzasED","ðŸ’° Finanzas ED"]]:[];})()),
                ["evaluacion","â­ Evaluac."],
                ["perfil","ðŸ‘¤ Perfil"],
              ].map(([id,label])=>(
            ...
            {masTab==="evaluacion"&&(
              <div>
                <h2 style={S.title}>Mi EvaluaciÃ³n</h2>
                {miEval?(
                  <div style={S.card}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                      <div><div style={{fontWeight:800,fontSize:16}}>{displayMaestroNombre(user.name)}</div><div style={{color:"#7B6B9A",fontSize:13}}>{teacherInfo.cargo} Â· {miClase}</div></div>
                    ...
                    {EVAL_KEYS.map((k,j)=>(
                      <div key={k} style={{marginBottom:12,background:k==="cumplimientoClases"?"linear-gradient(135deg,#5B2D8E18,#5B2D8E30)":"#F5F0FF",borderRadius:12,padding:"12px 14px",border:k==="cumplimientoClases"?"2px solid #5B2D8E55":"none"}}>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div style={{display:"flex",gap:2}}>{[1,2,3,4,5].map(s=><span key={s} style={{color:s<=(miEval[k]||0)?"#F5C842":"#e0e0e0",fontSize:20}}>â˜…</span>)}</div>
                          <span style={{fontWeight:800,color:miEval[k]>=4?"#4CAF50":miEval[k]>=3?"#F5A623":"#EF5350"}}>{miEval[k]||"â€”"}/5</span>
                        </div>
                      </div>
                    ))}
                    {(()=>{
                      ...
                      return(
                        <div style={{marginTop:16}}>
                          <div style={{fontWeight:800,fontSize:14,color:"#5B2D8E",marginBottom:8}}>ðŸ“‹ Historial de calificaciones</div>
                          {hayFallas&&(
                            <div style={{marginBottom:14}}>
                              <div style={{fontWeight:800,fontSize:12,color:"#EF5350",marginBottom:6}}>Fallas registradas (fecha, lecciÃ³n, grupo)</div>
                              <div style={{borderRadius:12,overflow:"hidden",border:"1px solid #EF535044"}}>
                                {fallasReg.map((e,i)=>(
                                  <div key={e.id||i} style={{display:"flex",alignItems:"center",padding:"8px 12px",background:i%2===0?"#FFF0F0":"#FFFFFF",fontSize:12}}>
                                    <span style={{color:"#EF5350",marginRight:8}}>âŒ</span>
                                    <span style={{fontWeight:600,color:"#2D1B4E"}}>{e.leccion||"â€”"}</span>
                                    <span style={{color:"#7B6B9A",marginLeft:8}}>ðŸ“… {formatFecha(e.fecha)} Â· {e.grupo||"â€”"}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {hayVideos&&(
                            <div>
                              <div style={{fontWeight:800,fontSize:12,color:"#4BBCE0",marginBottom:6}}>ðŸŽ¬ Calificaciones de videos (fecha, lecciÃ³n, grupo)</div>
                                ...
                                {rowsVideo.map((r,i)=>(
                                  <div key={r.ses.id||i} style={{display:"flex",alignItems:"center",padding:"8px 10px",background:i%2===0?"#F5F0FF":"#FFFFFF"}}>
                                    <div style={{flex:1,minWidth:0}}>
                                      <div style={{fontWeight:700,fontSize:13}}>{r.ses.leccion||"SesiÃ³n"}</div>
                                      <div style={{fontSize:11,color:"#7B6B9A"}}>{formatFecha(r.ses.fecha)} Â· {r.ses.grupo}</div>
                                    </div>
                                    <div style={{textAlign:"right",fontSize:11}}>
                                      <div>{r.vid?.hizo?"âœ… Enviado":"âŒ Sin video"}</div>
                ...
                ):<div style={{color:"#7B6B9A"}}>No hay evaluaciÃ³n disponible.</div>}
              </div>
            )}
            ...
            {masTab==="cumpleanos"&&(
              <div>
                <h2 style={S.title}>CumpleaÃ±os â€” {miClase}</h2>
                ...
                {(()=>{
                  ...
                  return todos.length===0?(<div style={{color:"#7B6B9A",textAlign:"center",padding:24,fontStyle:"italic"}}>No hay cumpleaÃ±os registrados en tu clase.</div>):(
                    todos.map((b,i)=>(
                      <div key={b.nombre+b.fecha+i} style={{...S.card,display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:12,color:"#7B6B9A"}}>{b.tipo}{b.edad?" Â· "+b.edad:""}</div>
                        </div>
              ...
              <div>
                ...
                <div style={{...S.card,textAlign:"center",padding:24}}>
                  <div style={{position:"relative",width:90,height:90,margin:"0 auto 14px"}}>
                    ...
                    <button onClick={()=>fileRef.current.click()} style={{position:"absolute",bottom:0,right:0,width:28,height:28,borderRadius:"50%",background:"#5B2D8E",border:"none",cursor:"pointer",fontSize:14,color:"#FFFFFF"}}>ðŸ“·</button>
                    <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={handlePhoto}/>
                  </div>
                  <div style={{fontWeight:800,fontSize:18}}>{displayMaestroNombre(user.name)}</div>
                  <div style={{color:"#7B6B9A",marginTop:4}}>{teacherInfo.cargo} Â· {miClase}</div>
                  {teacherInfo.cumpleanos&&<div style={{marginTop:8,color:"#E84F9B",fontWeight:700}}>ðŸŽ‚ {teacherInfo.cumpleanos}</div>}
                  <div style={{marginTop:10}}><span style={S.badge(teacherInfo.certificado==="SI"?"#4CAF50":"#EF5350")}>{teacherInfo.certificado==="SI"?"âœ… Certificado al dÃ­a":"âŒ Pendiente"}</span></div>
                </div>
                <div style={S.card}>
                  <h3 style={{color:"#5B2D8E",fontWeight:800,marginBottom:16,fontSize:16}}>ðŸ” Cambiar ContraseÃ±a</h3>
                  {pwOk?<div style={{color:"#4CAF50",fontWeight:700,textAlign:"center",padding:"20px 0"}}>âœ… ContraseÃ±a cambiada</div>:(
                    <>
                      {[["ContraseÃ±a actual","old"],["Nueva contraseÃ±a","new1"],["Confirmar nueva","new2"]].map(([l,k])=>(
                      ...
                      <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:14}} onClick={changePw}>Cambiar ContraseÃ±a</button>
                    </>
                  )}
                </div>
                <button style={{...S.btn("#EF5350","#FFFFFF",true),padding:14,marginTop:8}} onClick={onLogout}>ðŸšª Cerrar SesiÃ³n</button>
              </div>
            )}
          </div>
        )}
      </div>
      <Modal open={peticionModalT} onClose={()=>setPeticionModalT(false)} title="Editar PeticiÃ³n">
        ...
        <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:12}} onClick={savePeticionEditT}>ðŸ’¾ Guardar</button>
        <button style={{...S.btn("#FFF0F0","#EF5350"),padding:12,marginTop:10,border:"1.5px solid #EF535044"}} onClick={deletePeticionT}>ðŸ—‘ Eliminar</button>
      </Modal>
...
// â•â•â•â•â•â•â•â•â•â• PDF REPORTS â•â•â•â•â•â•â•â•â•â•
function generarPDF(titulo, htmlContent, options={}){
  ...
  const firmasHtml=firmas?`
    <div class="firmas">
      <div class="firma-item"><div class="firma-linea"></div><div class="firma-nombre">JosÃ© HernÃ¡n DÃ­az</div><div class="firma-cargo">Pastor</div></div>
      <div class="firma-item"><div class="firma-linea"></div><div class="firma-nombre">Cindy Vanessa MuÃ±oz</div><div class="firma-cargo">Directora de Escuela Dominical</div></div>
    </div>`:"";
  const full=`<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">${estilos}<title>${titulo}</title></head><body>
    <div class="header">
      <div>
        <h1>ðŸ“‹ ${titulo}</h1>
        <div class="sub">Escuela Dominical IPUE Â· Villanueva del Pardillo Â· ${fecha}</div>
      </div>
    </div>
    ${htmlContent}
    ${firmasHtml}
    <div class="footer">Generado por Sistema Escuela Dominical IPUE Â· ${fecha}</div>
    <button class="print-btn" onclick="window.print()">ðŸ–¨ï¸ Imprimir / Guardar PDF</button>
  </body></html>`;
...
function generarPDFLandscape(titulo, htmlContent){
  ...
  const full=`<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">${estilos}<title>${titulo}</title></head><body>
    <div class="header">
      <div><h1>ðŸ“… ${titulo}</h1><div class="sub">Escuela Dominical IPUE Â· Villanueva del Pardillo Â· ${fecha}</div></div>
    </div>
    ${htmlContent}
    <div class="footer">Generado por Sistema Escuela Dominical IPUE Â· ${fecha}</div>
    <button class="print-btn" onclick="window.print()">ðŸ–¨ï¸ Imprimir / Guardar PDF</button>
...
function InformesPanel({data}){
  ...
  const finanzasMiniSection=()=>{
    ...
    const filasGastos=(finanzas.gastos||[]).map(g=>{
      const resp=g.responsable?displayMaestroNombre(g.responsable):"â€”";
      return `<tr>
        <td>${g.fecha||"â€”"}</td>
        <td><strong>${g.concepto||"â€”"}</strong></td>
        <td>${resp}</td>
        <td style="font-weight:700;color:#C62828">â‚¬${(parseFloat(g.monto)||0).toFixed(2)}</td>
      </tr>`;
    }).join("");
    const filasActiv=(finanzas.actividades||[]).map(a=>{
      const resp=a.responsable?displayMaestroNombre(a.responsable):"â€”";
      const ef=parseFloat(a.ingresoEfectivo!=null?a.ingresoEfectivo:a.ingresos)||0;
      const tpv=parseFloat(a.ingresoTPV||0)||0;
      const ing=ef+tpv;
      ...
      return `<tr>
        <td>${a.fecha||"â€”"}</td>
        <td><strong>${a.nombre||"â€”"}</strong></td>
        <td>${resp}</td>
        <td style="color:#2E7D32">â‚¬${ing.toFixed(2)}</td>
        <td style="color:#C62828">â‚¬${gas.toFixed(2)}</td>
        <td style="font-weight:700;color:${net>=0?"#2E7D32":"#C62828"}">â‚¬${net.toFixed(2)}</td>
      </tr>`;
    }).join("");
    const filasDon=(finanzas.donativos||[]).map(d=>{
      const resp=d.responsable?displayMaestroNombre(d.responsable):"â€”";
      const ef=parseFloat(d.efectivo!=null?d.efectivo:d.monto)||0;
      const tpv=parseFloat(d.tpv||0)||0;
      const tot=ef+tpv;
      return `<tr>
        <td>${d.fecha||"â€”"}</td>
        <td><strong>${d.concepto||"â€”"}</strong></td>
        <td>${d.tipo||"DONATIVO"}</td>
        <td>${resp}</td>
        <td style="color:#2E7D32">â‚¬${ef.toFixed(2)}</td>
        <td style="color:#2E7D32">â‚¬${tpv.toFixed(2)}</td>
        <td style="font-weight:700;color:#2E7D32">â‚¬${tot.toFixed(2)}</td>
      </tr>`;
    }).join("");
    if(totalGastos===0&&totalIngresos===0&&!filasGastos&&!filasActiv&&!filasDon)return"";
    return `
      <div class="section">
        <div class="section-title">ðŸ’° Resumen de Finanzas (ComitÃ© + Clases)</div>
        <div style="padding:10px 16px 4px;font-size:12px;color:#2D1B4E;display:flex;flex-wrap:wrap;gap:16px">
          <div><div style="color:#7B6B9A;font-size:11px">Total gastos</div><div style="font-weight:900;color:#C62828;font-size:18px">â‚¬${totalGastos.toFixed(2)}</div></div>
          <div><div style="color:#7B6B9A;font-size:11px">Total ingresos</div><div style="font-weight:900;color:#2E7D32;font-size:18px">â‚¬${totalIngresos.toFixed(2)}</div></div>
          <div><div style="color:#7B6B9A;font-size:11px">Resultado neto</div><div style="font-weight:900;color:${neto>=0?"#2E7D32":"#C62828"};font-size:18px">â‚¬${neto.toFixed(2)}</div></div>
        </div>
        <div style="padding:4px 16px 0;font-size:11px;color:#7B6B9A">Incluye gastos de clases (meriendas / trabajos manuales), gastos generales del comitÃ©, actividades y donativos.</div>
        <div style="margin-top:10px">
          <h4 style="margin:8px 0 4px;font-size:12px;color:#5B2D8E">ðŸ›ï¸ Gastos generales del comitÃ©</h4>
          <table>
        ...
        <div style="margin-top:10px">
          <h4 style="margin:8px 0 4px;font-size:12px;color:#2A96BC">ðŸŽ‰ Actividades del comitÃ©</h4>
          <table>
        ...
        <div style="margin-top:10px">
          <h4 style="margin:8px 0 4px;font-size:12px;color:#2E7D32">ðŸ™ Donativos / Votos</h4>
          <table>
  ...
  // â”€â”€ Helper para mini grÃ¡ficos de barras â”€â”€
  ...
  const reportAlumnos=()=>{
    const rows=ninos.map(n=>{
      ...
      const bautizado=fam?.bautizado?"SÃ­":"No";
      const sellado=fam?.sellado?"SÃ­":"No";
      return `<tr>
        ...
        <td>${n.edad||"â€”"}</td>
        <td>${fam?.cumpleanos||"â€”"}</td>
        <td>${fam?.padre||fam?.madre?"âœ… SÃ­":"âš ï¸ No"}</td>
        <td>${bautizado}</td>
        <td>${sellado}</td>
        <td>${asistencias}/${totalSes} <small style="color:#7B6B9A">(${pct}%)</small></td>
        <td style="color:${scoreC};font-weight:800">${avg||"â€”"}</td>
        <td>${obsCount>0?obsCount+" obs.":"â€”"}</td>
      </tr>`;
    }).join("");
    ...
    const html=`
      <div class="section">
        <div class="section-title">ðŸ“Š Resumen General Â· ${ninos.length} Alumnos</div>
        <div style="padding:10px 16px 4px">
          ${barRow("Bautizados",totalBaut,ninos.length,"#4CAF50")}
          ${barRow("Sellados",totalSell,ninos.length,"#5B2D8E")}
        </div>
        <table>
          <thead><tr><th>Alumno</th><th>Clase</th><th>Edad</th><th>CumpleaÃ±os</th><th>Familia</th><th>Bautizado</th><th>Sellado</th><th>Asistencia</th><th>Promedio</th><th>Observac.</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
  ...
  // â”€â”€ Report states (hooks must be at top level) â”€â”€
  ...
  // â”€â”€ Report 2: Individual Teacher â”€â”€
  const reportMaestro=()=>{
    ...
    const sesRows=sesiones.map(s=>{
      ...
      <td><strong>${s.leccion||"â€”"}</strong></td>
      <td>${s.tema||"â€”"}</td>
      <td><span class="${s.maestro===m.nombre?"presente":"ausente"}">${s.maestro===m.nombre?"ðŸŽ“ Maestro":"ðŸ¤ Auxiliar"}</span></td>
      <td>${gastos>0?"â‚¬"+gastos.toFixed(2):"â€”"}${donativoNote}</td>
    </tr>`;
    }).join("");
    ...
    const evalRows=ev?EVAL_KEYS.map((k,j)=>`<tr><td>${EVAL_LABELS[j]}</td><td>${"â˜…".repeat(ev[k]||0)}${"â˜†".repeat(5-(ev[k]||0))}</td><td style="font-weight:700">${ev[k]||0}/5</td></tr>`).join(""):"";
    ...
    const videoRows=sesionesConVideo.map(s=>{
      ...
      return`<tr>
        ...
        <td><strong>${s.leccion||"â€”"}</strong></td>
        <td>${s.grupo}</td>
        <td>${v?.hizo?"âœ… SÃ­":"âŒ No"}</td>
        <td>${v?.hizo?(v.calidad||"â€”")+"/5":"â€”"}</td>
        <td>${v?.hizo?(v.aTiempo?"âœ… SÃ­":"No"):"â€”"}</td>
        <td style="font-weight:700">${score!=null?score.toFixed(1)+"/5":"â€”"}</td>
      </tr>`;
    }).join("");
    const videoSection=sesionesConVideo.length>0?`
      <div class="section">
        <div class="section-title">ðŸŽ¬ EnvÃ­o de Videos de Clase</div>
        <p style="font-size:11px;color:#7B6B9A;padding:8px 16px 0">Videos requeridos para clases Corderitos, Vencedores y Conquistadores. Calidad 1-5 y envÃ­o a tiempo.</p>
        <table><thead><tr><th>Fecha</th><th>LecciÃ³n</th><th>Grupo</th><th>EnviÃ³</th><th>Calidad</th><th>A tiempo</th><th>Score</th></tr></thead><tbody>${videoRows}</tbody></table>
      </div>`:"";
    ...
    const html=`
      <div class="card">
        <div style="display:flex;align-items:center;margin-bottom:10px">
          ...
          <div>
            ...
            <div class="card-sub">${m.cargo} Â· Clase ${m.clase} Â· Certificado: ${m.certificado||"â€”"}</div>
          </div>
        </div>
        ...
        <div class="grid2">
          ...
          ${avgEv!=null?`<div class="stat"><div class="stat-val">${typeof avgEv==="number"?avgEv.toFixed(1):avgEv}/5</div><div class="stat-lbl">EvaluaciÃ³n (incl. videos)</div></div>`:""}
          <div class="stat"><div class="stat-val">${misNinos.length}</div><div class="stat-lbl">Alumnos en clase</div></div>
        </div>
      </div>
      <div class="section">
        <div class="section-title">ðŸ“… Historial de Clases</div>
        <table><thead><tr><th>Fecha</th><th>LecciÃ³n</th><th>Tema</th><th>Rol</th><th>Gastos</th></tr></thead><tbody>${sesRows||"<tr><td colspan='5' style='text-align:center;color:#AAA'>Sin clases asignadas</td></tr>"}</tbody></table>
      </div>
      ${videoSection}
      <div class="section">
        <div class="section-title">ðŸ‘¦ Alumnos de ${m.clase}</div>
      ...
      ${ev?`<div class="section"><div class="section-title">â­ EvaluaciÃ³n del Administrador</div><table><thead><tr><th>Criterio</th><th>CalificaciÃ³n</th><th>Puntos</th></tr></thead><tbody>${evalRows}</tbody></table>${(ev.observaciones||"").trim()?`<div style="padding:12px 16px;border-top:1px solid #DDD0F0"><div style="font-size:11px;font-weight:700;color:#5B2D8E;margin-bottom:6px">Observaciones</div><div style="font-size:13px;color:#2D1B4E;white-space:pre-wrap">${String(ev.observaciones||"").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div>`:""}</div>`:""}`+finanzasMiniSection();
    generarPDF(`Informe de ${displayMaestroNombre(m.nombre)}`,html);
  };

  // â”€â”€ Report 4: Individual Student â”€â”€
  const reportNino=()=>{
    ...
    const sesRows=entries.map(e=>{
      ...
      return"<tr><td>"+formatFecha(e.fecha)+"</td><td><strong>"+(e.leccion||"â€”")+"</strong></td>"+
        ckeys.map(k=>"<td style=\"color:"+scoreColorHex(e[k])+";font-weight:700\">"+(e[k]||"â€”")+"</td>").join("")+
        "<td style=\"color:"+(ea?scoreColorHex(ea):"#AAA")+";font-weight:800\">"+(ea||"â€”")+"</td>"+
    ...
    const obsRows=entries.filter(e=>e.observacion).map(e=>
      ...
      "<div style=\"font-size:11px;color:#5B2D8E;font-weight:700\">"+(e.leccion||"â€”")+" Â· "+formatFecha(e.fecha)+"</div>"+
      "<div style=\"font-size:12px;color:#2D1B4E\">"+(e.observacion)+"</div></div>"
    ).join("");
    const padreRow=fam&&fam.padre&&fam.padre!=="(No registra)"?"<div style=\"font-size:12px;color:#7B6B9A\">ðŸ‘¨ "+fam.padre+(fam.telPadre?" Â· ðŸ“ž "+fam.telPadre:"")+"</div>":"";
    const madreRow=fam&&fam.madre&&fam.madre!=="(No registra)"?"<div style=\"font-size:12px;color:#7B6B9A\">ðŸ‘© "+fam.madre+(fam.telMadre?" Â· ðŸ“ž "+fam.telMadre:"")+"</div>":"";
    const bautizadoSellado="<div style=\"margin-top:8px;font-size:12px\">"+(fam?.bautizado?"<span style=\"color:#2E7D32;font-weight:700\">ðŸ™ Bautizado</span>":"<span style=\"color:#7B6B9A\">Bautizado: No</span>")+" &nbsp; "+(fam?.sellado?"<span style=\"color:#5B2D8E;font-weight:700\">âœ¨ Sellado</span>":"<span style=\"color:#7B6B9A\">Sellado: No</span>")+"</div>";
    ...
    const historial=entries.length>0?
      "<div class=\"section\"><div class=\"section-title\">ðŸ“Š Historial de Calificaciones ("+entries.length+" sesiones)</div>"+
      "<table><thead><tr><th>Fecha</th><th>LecciÃ³n</th>"+crit.map(c=>"<th>"+c.logro+"</th>").join("")+"<th>Prom.</th><th>ObservaciÃ³n</th></tr></thead>"+
      "<tbody>"+sesRows+"</tbody></table></div>"+
      (obsRows?"<div class=\"section\"><div class=\"section-title\">ðŸ’¬ Observaciones</div>"+obsRows+"</div>":"")
    ...
    const html=
      ...
      "<div style=\"font-size:12px;color:#7B6B9A\">Clase: <strong>"+n.clase+"</strong> Â· Edad: <strong>"+(n.edad||"â€”")+"</strong></div>"+
      ...
      "<div class=\"stat\"><div class=\"stat-val\" style=\"color:"+scoreC+"\">"+( avg||"â€”")+"</div><div class=\"stat-lbl\">Promedio general</div></div>"+
      "</div></div>"+historial+finanzasMiniSection();
    generarPDF("Informe: "+displayNameAlumno(n),html);
  };

  // â”€â”€ Report 3: Class Summary â”€â”€
  ...
  const reportClase=()=>{
    ...
    const sesRows=claseSesiones.map(s=>{
      ...
      return`<tr>
        ...
        <td style="color:${avg2?scoreColorHex(avg2):"#AAA"};font-weight:800">${avg2||"â€”"}</td>
        <td style="color:${gasto>0?"#2E7D32":"#AAA"}">${gasto>0?"â‚¬"+gasto.toFixed(2):"â€”"}${donativoNote2}</td>
      </tr>`;
    }).join("");
    const ninoRows2=claseNinos.map(n=>{
      ...
      return`<tr>
        ...
        <td>${n.edad||"â€”"}</td>
        <td>${fam?.cumpleanos||"â€”"}</td>
        <td>${fam?.bautizado?"SÃ­":"No"}</td>
        <td>${fam?.sellado?"SÃ­":"No"}</td>
        <td>${asist}/${claseSesiones.length}</td>
        <td style="color:${avg2?scoreColorHex(avg2):"#AAA"};font-weight:800">${avg2||"â€”"}</td>
        <td>${obsCount>0?obsCount+" obs.":"â€”"}</td>
      </tr>`;
    }).join("");
    const promedioClase=(()=>{let t=0,cnt=0;claseNinos.forEach(n=>{const a=ninoAvg(n.nombre,selClase);if(a){t+=parseFloat(a);cnt++;}});return cnt?(t/cnt).toFixed(1):"â€”";})();
    ...
    const html=`
      <div class="card">
        <div class="card-title" style="color:${claseColorHex(selClase)};font-size:18px">ðŸ« Clase ${selClase}</div>
        <div class="card-sub">${claseMaestro?`Maestro: ${displayMaestroNombre(claseMaestro.nombre)}`:"Sin maestro asignado"} ${claseAux.length?`Â· Auxiliar(es): ${claseAux.map(a=>displayMaestroNombre(a.nombre)).join(", ")}`:""}
        </div>
        <div class="grid2">
          ...
          <div class="stat"><div class="stat-val" style="color:#2E7D32">â‚¬${totalGasto.toFixed(2)}</div><div class="stat-lbl">Gasto total</div></div>
      ...
      <div class="section">
        <div class="section-title">ðŸ“… Sesiones de Clase</div>
        <table><thead><tr><th>Fecha</th><th>LecciÃ³n</th><th>Tema</th><th>Asistencia</th><th>Promedio</th><th>Gasto</th></tr></thead>
        <tbody>${sesRows||"<tr><td colspan='6' style='text-align:center;color:#AAA'>Sin sesiones registradas</td></tr>"}</tbody></table>
      </div>
      <div class="section">
        <div class="section-title">ðŸ‘¦ Alumnos</div>
        <table><thead><tr><th>Alumno</th><th>Edad</th><th>CumpleaÃ±os</th><th>Bautizado</th><th>Sellado</th><th>Asistencia</th><th>Promedio</th><th>Observac.</th></tr></thead>
  ...
  // â”€â”€ Report 5: Economic Summary â”€â”€
  const reportEconomico=()=>{
    ...
    if(totalGastos===0&&totalRecaudado===0){
      const html=`<div class="section"><div class="section-title">ðŸ“Š Informe EconÃ³mico</div><p style="padding:20px;font-size:13px;color:#7B6B9A;font-style:italic">No hay movimientos econÃ³micos registrados todavÃ­a.</p></div>`;
      generarPDF("Informe EconÃ³mico",html);
      return;
    }
    const resumenGlobal=`
      <div class="section">
        <div class="section-title">ðŸ’° Resumen EconÃ³mico General</div>
        <div style="padding:12px 18px;font-size:13px;color:#2D1B4E;">
          <div style="font-weight:800;margin-bottom:12px;color:#5B2D8E">ðŸ“¥ Total recaudado</div>
          <div style="display:flex;flex-wrap:wrap;gap:16px 24px;margin-bottom:8px">
            <span>Actividades: <strong style="color:#2E7D32">â‚¬${ingresosActividades.toFixed(2)}</strong></span>
            <span>Votos: <strong style="color:#2E7D32">â‚¬${votosTotal.toFixed(2)}</strong></span>
            <span>Donativos: <strong style="color:#2E7D32">â‚¬${donativosSoloTotal.toFixed(2)}</strong></span>
            <span>Ofrenda niÃ±os E.D.: <strong style="color:#2E7D32">â‚¬${ofrendaNinosTotal.toFixed(2)}</strong></span>
          </div>
          <div style="font-size:18px;font-weight:900;color:#2E7D32;margin-bottom:16px;border-top:1px solid #DDD0F0;padding-top:10px">Total recaudado: â‚¬${totalRecaudado.toFixed(2)}</div>
          <div style="font-weight:800;margin-bottom:12px;color:#5B2D8E">ðŸ“¤ Total gastado (restando donativos de meriendas/t. manual)</div>
          <div style="display:flex;flex-wrap:wrap;gap:16px 24px;margin-bottom:8px">
            <span>Clases (neto): <strong style="color:#C62828">â‚¬${gastosMeriendasNeto.toFixed(2)}</strong>${donativosMeriendasTotal>0?` <small style="color:#7B6B9A">(donativos descontados: â‚¬${donativosMeriendasTotal.toFixed(2)})</small>`:""}</span>
            <span>ComitÃ©: <strong style="color:#C62828">â‚¬${gastosComite.toFixed(2)}</strong></span>
            <span>Actividades: <strong style="color:#C62828">â‚¬${gastosActividades.toFixed(2)}</strong></span>
          </div>
          <div style="font-size:18px;font-weight:900;color:#C62828;margin-bottom:16px;border-top:1px solid #DDD0F0;padding-top:10px">Total gastado: â‚¬${totalGastos.toFixed(2)}</div>
          <div style="font-weight:800;font-size:20px;color:${balanceTotal>=0?"#2E7D32":"#C62828"};border:2px solid ${balanceTotal>=0?"#2E7D32":"#C62828"};border-radius:12px;padding:14px;text-align:center">Balance total: â‚¬${balanceTotal.toFixed(2)}</div>
    ...
    const seccionesClases=CLASES_LIST.map(cl=>{
      ...
      const filasSesiones=merClase.map(m=>{
        ...
        const maestro=ses.maestro?displayMaestroNombre(ses.maestro):"â€”";
        const aux=ses.auxiliar?displayMaestroNombre(ses.auxiliar):"â€”";
        ...
        return `<tr>
          ...
          <td>${maestro}</td>
          <td>${aux}</td>
          <td>${merTxt||merCost>0?"ðŸŽ "+(merTxt||"Merienda"):"â€”"}${merCost>0?" Â· â‚¬"+merCost.toFixed(2):""}${merDonativo}</td>
          <td>${tmTxt||tmCost>0?"âœ‚ï¸ "+(tmTxt||"T. manual"):"â€”"}${tmCost>0?" Â· â‚¬"+tmCost.toFixed(2):""}${tmDonativo}</td>
          <td style="font-weight:700;color:${total>0?"#2E7D32":"#7B6B9A"}">${total>0?"â‚¬"+total.toFixed(2):"â€”"}</td>
        </tr>`;
      }).join("");
      ...
      const filasMaestros=Object.entries(porMaestro).sort((a,b)=>b[1]-a[1]).map(([nombre,total])=>{
        ...
        return `<tr>
          <td>${short}</td>
          <td>${cargo}</td>
          <td style="font-weight:800;color:#2E7D32">â‚¬${total.toFixed(2)}</td>
        </tr>`;
      }).join("");

      return `
        <div class="section">
          <div class="section-title">ðŸ« Clase ${cl}</div>
          <div style="padding:8px 16px;font-size:12px;color:#7B6B9A">Total de gastos registrados en esta clase: <strong style="color:#2E7D32">â‚¬${totalClase.toFixed(2)}</strong></div>
          <table>
            <thead><tr><th>Fecha</th><th>LecciÃ³n</th><th>Maestro</th><th>Auxiliar</th><th>Merienda</th><th>Trabajo Manual</th><th>Total</th></tr></thead>
            <tbody>${filasSesiones||"<tr><td colspan='7' style='text-align:center;color:#AAA'>Sin gastos registrados</td></tr>"}</tbody>
          </table>
          <div style="margin-top:16px">
            <div style="font-weight:800;font-size:13px;margin-bottom:6px;color:#2D1B4E">ðŸ‘©â€ðŸ« Resumen por maestro (solo rol de maestro)</div>
            <table>
    ...
    const actividadesRows=(finanzas.actividades||[]).map(a=>{
      ...
      return `<tr>
        ...
        <td><strong>${a.nombre||"â€”"}</strong></td>
        <td>${resp}</td>
        <td style="color:#2E7D32">â‚¬${ingEf.toFixed(2)}</td>
        <td style="color:#2E7D32">â‚¬${ingTpv.toFixed(2)}</td>
        <td style="color:#2E7D32">â‚¬${ing.toFixed(2)}</td>
        <td style="color:#C62828">â‚¬${gas.toFixed(2)}</td>
        <td style="font-weight:700;color:${net>=0?"#2E7D32":"#C62828"}">â‚¬${net.toFixed(2)}</td>
      </tr>`;
    ...
    const donativosRows=(finanzas.donativos||[]).map(d=>{
      ...
      return `<tr>
        ...
        <td style="color:#2E7D32">â‚¬${ef.toFixed(2)}</td>
        <td style="color:#2E7D32">â‚¬${tpv.toFixed(2)}</td>
        <td style="font-weight:700;color:#2E7D32">â‚¬${total.toFixed(2)}</td>
      </tr>`;
    }).join("");

    const ofrendaNinosRows=(finanzas.ofrendaNinos||[]).slice().sort((a,b)=>(b.fecha||"").localeCompare(a.fecha||"")).map(o=>{
      const resp=o.responsable?displayMaestroNombre(o.responsable):"â€”";
      return `<tr>
        <td>${o.fecha||"â€”"}</td>
        <td style="font-weight:700;color:#2E7D32">â‚¬${(parseFloat(o.monto)||0).toFixed(2)}</td>
        <td>${resp}</td>
      </tr>`;
    }).join("");
    ...
    const seccionComite=`
      <div class="section">
        <div class="section-title">ðŸ›ï¸ ComitÃ© de Escuela Dominical â€” Gastos Generales</div>
        <table>
          <thead><tr><th>Fecha</th><th>Concepto</th><th>Responsable</th><th>Monto</th></tr></thead>
    ...
    const seccionActividades=`
      <div class="section">
        <div class="section-title">ðŸŽ‰ Actividades del ComitÃ©</div>
        <table>
  ...
  const seccionDonativos=`
      <div class="section">
        <div class="section-title">ðŸ™ Donativos / Votos</div>
        <table>
          <thead><tr><th>Fecha</th><th>Detalle</th><th>Tipo</th><th>Responsable</th><th>Efec.</th><th>TPV</th><th>Total</th></tr></thead>
    ...
    const seccionOfrendaNinos=`
      <div class="section">
        <div class="section-title">ðŸ‘§ Ofrenda de niÃ±os Escuela Dominical</div>
        <table>
  ...
  // â”€â”€ Report 6: ProgramaciÃ³n mensual en PDF (cronograma detallado por dÃ­a y clase) â”€â”€
  const reportCalendarioMensual=()=>{
    ...
    if(!year||!month||month<1||month>12){
      alert("Selecciona un mes vÃ¡lido.");
      return;
    }
    ...
    const weekdayNamesLong=["Domingo","Lunes","Martes","MiÃ©rcoles","Jueves","Viernes","SÃ¡bado"];
    ...
    if(dates.length===0){
      alert("No hay programaciÃ³n con clase para "+monthLabel+".");
      return;
    }
    ...
    const sections=dates.map(fecha=>{
      ...
      const rows=byDate[fecha]
        ...
        .map(e=>{
          ...
          const maestro=e.maestro?displayMaestroNombre(e.maestro):"â€”";
          const aux=e.auxiliar?displayMaestroNombre(e.auxiliar):"â€”";
          const tema=(e.tema||"").trim();
          const temaStr=tema||"â€”";
          return `<tr>
            <td><span class="badge" style="background:${color}15;color:${color};border:1px solid ${color}55">${e.grupo}</span></td>
            <td><strong>${e.leccion||"â€”"}</strong></td>
            <td>${temaStr}</td>
            <td>${maestro}</td>
            <td>${aux}</td>
          </tr>`;
      ...
      return `
        <div class="section">
          <div class="section-title">ðŸ“… ${dateStr}</div>
          <table>
            <thead>
              <tr>
                <th>Grupo</th>
    ...
    const html=sections;
    generarPDF("ProgramaciÃ³n "+monthLabel,html,{firmas:false});
  };

  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      <h2 style={S.title}>ðŸ“„ Generar Informes PDF</h2>
      <div style={{background:"#F0FBFF",borderRadius:14,padding:"14px",marginBottom:18,fontSize:13,color:"#2A96BC",fontWeight:600}}>
        Los informes se abrirÃ¡n en una nueva pestaÃ±a listos para imprimir o guardar como PDF.
      </div>

      {/* Report 1 - All Students */}
      <div style={{...S.card,borderLeft:"5px solid #5B2D8E",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#5B2D8E",marginBottom:4}}>ðŸ“‹ Informe de Alumnos</div>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:14}}>Todos los alumnos con asistencia, promedio, cumpleaÃ±os y datos de familia.</div>
        <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportAlumnos}>
          ðŸ“¥ Generar PDF â€” Todos los Alumnos ({ninos.length})
        </button>
      </div>

      {/* Report 2 - Individual Teacher */}
      <div style={{...S.card,borderLeft:"5px solid #E84F9B",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#E84F9B",marginBottom:4}}>ðŸ‘©â€ðŸ« Informe Individual de Maestro/Auxiliar</div>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:12}}>Historial de clases, alumnos, evaluaciÃ³n y estadÃ­sticas personales.</div>
        ...
        <select style={{...S.input,marginBottom:14}} value={selMaestro} onChange={e=>setSelMaestro(e.target.value)}>
          {[...maestros].sort((a,b)=>sortKeyFirstName(a.nombre).localeCompare(sortKeyFirstName(b.nombre),"es")).map(m=>(
            <option key={m.id} value={m.nombre}>{displayMaestroNombre(m.nombre)} â€” {m.cargo} ({m.clase})</option>
          ))}
        </select>
        <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportMaestro}>
          ðŸ“¥ Generar PDF â€” {selMaestro?displayMaestroNombre(selMaestro):"Selecciona uno"}
        </button>
      </div>

      {/* Report 3 - Class Summary */}
      <div style={{...S.card,borderLeft:"5px solid #4BBCE0",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#2A96BC",marginBottom:4}}>ðŸ« Informe General de Clase</div>
        ...
        <button style={{...S.btn("#4BBCE0","#FFFFFF",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportClase}>
          ðŸ“¥ Generar PDF â€” Clase {selClase} ({(clases[selClase]||[]).length} alumnos)
        </button>
      </div>

      {/* Report 4 - Individual Student */}
      <div style={{...S.card,borderLeft:"5px solid #4CAF50",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#2E7D32",marginBottom:4}}>ðŸ‘¦ Informe Individual de Alumno</div>
        ...
        <select style={{...S.input,marginBottom:14}} value={selNino} onChange={e=>setSelNino(e.target.value)}>
          {ninos.sort((a,b)=>sortKeyFirstName(a.nombre).localeCompare(sortKeyFirstName(b.nombre),"es")).map(n=>(
            <option key={n.nombre+n.clase} value={n.nombre}>{displayNameAlumno(n)} â€” {n.clase}</option>
          ))}
        </select>
        <button style={{...S.btn("#4CAF50","#FFFFFF",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportNino}>
          ðŸ“¥ Generar Informe â€” {selNino?shortDisplayName(selNino):"Selecciona uno"}
        </button>
      </div>
      {/* Report 5 - Economic Summary */}
      <div style={{...S.card,borderLeft:"5px solid #F5C842",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#B69100",marginBottom:4}}>ðŸ’° Informe EconÃ³mico General</div>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:12}}>Resumen de gastos de clases, gastos del comitÃ©, actividades y donativos.</div>
        <button style={{...S.btn("#F5C842","#3D1B6B",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportEconomico}>
          ðŸ“¥ Generar Informe EconÃ³mico
        </button>
      </div>

      {/* Report 6 - ProgramaciÃ³n mensual (PDF detallado por dÃ­a y clase) */}
      <div style={{...S.card,borderLeft:"5px solid #7E57C2"}}>
        <div style={{fontWeight:800,fontSize:15,color:"#5E35B1",marginBottom:4}}>ðŸ“… ProgramaciÃ³n del mes</div>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:12}}>Informe PDF por dÃ­a: fecha como tÃ­tulo y tabla Grupo, LecciÃ³n, Tema, Maestro y Auxiliar.</div>
        ...
        <button style={{...S.btn("#7E57C2","#FFFFFF",true),padding:"12px 20px",borderRadius:12,fontSize:14,width:"100%"}} onClick={reportCalendarioMensual}>
          ðŸ“¥ Generar PDF â€” {calendarYearMonth?calendarYearMonth.replace("-","/")+"":""}
        </button>
      </div>
    </div>
  );
...
// â•â•â•â•â•â•â•â•â•â• PETICIONES DE ORACIÃ“N â•â•â•â•â•â•â•â•â•â•
function PeticionesDashboard({peticiones,user,onUpdate}){
  ...
  return(
    <div style={{...S.card,borderLeft:"5px solid #E84F9B",margin:"0 0 14px"}}>
      <div style={{fontWeight:800,color:"#E84F9B",fontSize:15,marginBottom:10}}>ðŸ™ Peticiones de OraciÃ³n</div>
      {recent.map((p,i)=>(
        <div key={p.id} style={{padding:"8px 0",borderBottom:i<recent.length-1?"1px solid #DDD0F0":"none"}}>
          ...
          <div style={{fontSize:11,color:"#7B6B9A",marginTop:3}}>
            {p.anonimo?"ðŸ”’ AnÃ³nimo":shortDisplayName(p.autor)} Â· {p.fecha?new Date(p.fecha).toLocaleDateString("es-ES",{day:"2-digit",month:"short"}):""}
          </div>
        </div>
      ))}
    </div>
...
function PeticionesPanel({peticiones,user,onUpdate,isAdmin=false}){
  ...
  return(
    <div style={{padding:"16px 0 20px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <h2 style={{...S.title,margin:0}}>ðŸ™ Peticiones</h2>
        <button style={{...S.btn("#E84F9B"),padding:"10px 16px",fontSize:14}} onClick={openAdd}>+ Nueva</button>
      </div>
      {sorted.length===0&&<div style={{color:"#7B6B9A",textAlign:"center",padding:30,fontStyle:"italic"}}>Sin peticiones. Â¡AÃ±ade la primera!</div>}
      {sorted.map(p=>(
        <div key={p.id} style={{...S.card,borderLeft:"5px solid #E84F9B",padding:"14px 16px"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
            <div style={{fontSize:12,color:"#7B6B9A",display:"flex",alignItems:"center",gap:6}}>
              {p.anonimo?<span style={{...S.badge("#7B6B9A"),fontSize:11}}>ðŸ”’ AnÃ³nimo</span>:<span style={{...S.badge("#5B2D8E"),fontSize:11}}>ðŸ‘¤ {shortDisplayName(p.autor)}</span>}
              <span>Â· {p.fecha?new Date(p.fecha).toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):""}</span>
            </div>
            {canEdit(p)&&(
              <button style={{...S.btn("#4BBCE0"),padding:"5px 10px",fontSize:12}} onClick={()=>openEdit(p)}>âœï¸</button>
            )}
          </div>
        </div>
      ))}
      <Modal open={modal} onClose={()=>setModal(false)} title={editId?"Editar PeticiÃ³n":"Nueva PeticiÃ³n de OraciÃ³n"}>
        <div style={{background:"#FFF0F8",borderRadius:12,padding:"12px 14px",marginBottom:14,fontSize:13,color:"#7B6B9A"}}>Las peticiones son visibles para todos los maestros y auxiliares. Puedes hacerla anÃ³nima si lo prefieres.</div>
        <label style={S.label}>PeticiÃ³n</label>
        <textarea style={{...S.input,height:100,resize:"vertical",marginBottom:16}} value={form.texto} onChange={e=>setForm(f=>({...f,texto:e.target.value}))} placeholder="Escribe tu peticiÃ³n de oraciÃ³n..."/>
        <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",marginBottom:20,padding:"12px 14px",background:"#F5F0FF",borderRadius:12}}>
          ...
          <div>
            <div style={{fontWeight:700,fontSize:14}}>ðŸ”’ PeticiÃ³n anÃ³nima</div>
            <div style={{fontSize:12,color:"#7B6B9A"}}>Tu nombre no aparecerÃ¡</div>
          </div>
        </label>
        <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:14}} onClick={save}>ðŸ™ Guardar PeticiÃ³n</button>
        {editId&&<button style={{...S.btn("#FFF0F0","#EF5350"),padding:12,marginTop:10,border:"1.5px solid #EF535044"}} onClick={()=>{if(confirmDelete("Â¿Eliminar esta peticiÃ³n de oraciÃ³n?")){onUpdate((peticiones||[]).filter(x=>x.id!==editId));setModal(false);}}}>ðŸ—‘ Eliminar PeticiÃ³n</button>}
      </Modal>
    </div>
  );
}

// â•â•â•â•â•â•â•â•â•â• FINANZAS PANEL (ADMIN) â•â•â•â•â•â•â•â•â•â•
function FinanzasPanel({finanzas,maestros,onUpdate}){
  ...
  const deleteGasto=id=>{
    if(!confirmDelete("Â¿Eliminar este gasto del comitÃ©?"))return;
  ...
  const deleteActividad=id=>{
    if(!confirmDelete("Â¿Eliminar esta actividad del comitÃ©?"))return;
  ...
  const deleteDonativo=id=>{
    if(!confirmDelete("Â¿Eliminar este registro de donativo/voto?"))return;
  ...
  const deleteOfrendaNinos=id=>{
    if(!confirmDelete("Â¿Eliminar este registro de ofrenda?"))return;
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      <h2 style={S.title}>ðŸ’° Finanzas del ComitÃ©</h2>

      <div style={{...S.card,borderLeft:"5px solid #5B2D8E",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#5B2D8E",marginBottom:4}}>ðŸ‘§ Ofrenda de niÃ±os Escuela Dominical</div>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:10}}>Registro de las ofrendas que entran cada domingo. Indica la fecha y quiÃ©n la registrÃ³ (maestro/auxiliar).</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8,marginBottom:10}}>
          ...
          <input style={S.input} placeholder="â‚¬ Monto" value={ofrendaForm.monto} onChange={e=>setOfrendaForm(f=>({...f,monto:e.target.value}))}/>
          <select style={S.input} value={ofrendaForm.responsable} onChange={e=>setOfrendaForm(f=>({...f,responsable:e.target.value}))}>
            <option value="">QuiÃ©n registrÃ³</option>
            {maestrosOpts.map(m=><option key={m.id} value={m.nombre}>{displayMaestroNombre(m.nombre)} â€” {m.cargo}</option>)}
          </select>
        </div>
        <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:10,fontSize:13,marginBottom:10}} onClick={saveOfrendaNinos}>âž• AÃ±adir ofrenda</button>
        ...
        {ofrendaNinos.slice().sort((a,b)=>(b.fecha||"").localeCompare(a.fecha||"")).map(o=>(
          <div key={o.id} style={{marginTop:8,padding:"8px 10px",borderRadius:10,background:"#F5F0FF",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
            <div style={{fontSize:12,color:"#2D1B4E"}}>
              <div style={{fontWeight:700}}>â‚¬{(parseFloat(o.monto)||0).toFixed(2)}</div>
              <div style={{fontSize:11,color:"#7B6B9A"}}>{o.fecha} Â· Registrado por: {o.responsable?displayMaestroNombre(o.responsable):"â€”"}</div>
            </div>
            <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 8px",fontSize:11}} onClick={()=>deleteOfrendaNinos(o.id)}>ðŸ—‘</button>
          </div>
        ))}
      </div>

      <div style={{...S.card,borderLeft:"5px solid #E84F9B",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#E84F9B",marginBottom:4}}>ðŸ›ï¸ Gastos del ComitÃ©</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8,marginBottom:10}}>
          ...
          <input style={S.input} placeholder="â‚¬ Monto" value={gastoForm.monto} onChange={e=>setGastoForm(f=>({...f,monto:e.target.value}))}/>
          <select style={S.input} value={gastoForm.responsable} onChange={e=>setGastoForm(f=>({...f,responsable:e.target.value}))}>
            <option value="">Responsable</option>
            {maestrosOpts.map(m=><option key={m.id} value={m.nombre}>{displayMaestroNombre(m.nombre)} â€” {m.cargo}</option>)}
          </select>
        </div>
        <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:10,fontSize:13,marginBottom:10}} onClick={saveGasto}>âž• AÃ±adir gasto</button>
        ...
        {gastos.slice().sort((a,b)=>(b.fecha||"").localeCompare(a.fecha||"")).map(g=>(
          <div key={g.id} style={{marginTop:8,padding:"8px 10px",borderRadius:10,background:"#FDF5F7",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
            <div style={{fontSize:12,color:"#2D1B4E"}}>
              ...
              <div style={{fontSize:11,color:"#7B6B9A"}}>{g.fecha} Â· {g.responsable?displayMaestroNombre(g.responsable):"Sin responsable"}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontWeight:800,color:"#C62828",fontSize:13}}>â‚¬{(parseFloat(g.monto)||0).toFixed(2)}</span>
              <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 8px",fontSize:11}} onClick={()=>deleteGasto(g.id)}>ðŸ—‘</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{...S.card,borderLeft:"5px solid #4BBCE0",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#2A96BC",marginBottom:4}}>ðŸŽ‰ Actividades del ComitÃ©</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginBottom:10}}>
          ...
          <input style={S.input} placeholder="â‚¬ Efectivo" value={actForm.ingresoEfectivo} onChange={e=>setActForm(f=>({...f,ingresoEfectivo:e.target.value}))}/>
          <input style={S.input} placeholder="â‚¬ TPV" value={actForm.ingresoTPV} onChange={e=>setActForm(f=>({...f,ingresoTPV:e.target.value}))}/>
          <input style={S.input} placeholder="â‚¬ Gastos" value={actForm.gastos} onChange={e=>setActForm(f=>({...f,gastos:e.target.value}))}/>
          <select style={S.input} value={actForm.responsable} onChange={e=>setActForm(f=>({...f,responsable:e.target.value}))}>
            <option value="">Responsable</option>
            {maestrosOpts.map(m=><option key={m.id} value={m.nombre}>{displayMaestroNombre(m.nombre)} â€” {m.cargo}</option>)}
          </select>
        </div>
        <button style={{...S.btn("#4BBCE0","#FFFFFF",true),padding:10,fontSize:13,marginBottom:10}} onClick={saveActividad}>âž• AÃ±adir actividad</button>
          ...
          return(
            <div key={a.id} style={{marginTop:8,padding:"8px 10px",borderRadius:10,background:"#F0FAFF",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div style={{fontSize:12,color:"#2D1B4E"}}>
                ...
                <div style={{fontSize:11,color:"#7B6B9A"}}>{a.fecha} Â· {a.responsable?displayMaestroNombre(a.responsable):"Sin responsable"}</div>
              </div>
              <div style={{textAlign:"right",fontSize:11}}>
                <div style={{color:"#2E7D32",fontWeight:700}}>Efec.: â‚¬{ingEf.toFixed(2)} Â· TPV: â‚¬{ingTpv.toFixed(2)}</div>
                <div style={{color:"#2E7D32",fontWeight:700}}>Ingresos totales: â‚¬{ing.toFixed(2)}</div>
                <div style={{color:"#C62828",fontWeight:700}}>Gastos: â‚¬{gas.toFixed(2)}</div>
                <div style={{fontWeight:800,color:net>=0?"#2E7D32":"#C62828"}}>Neto: â‚¬{net.toFixed(2)}</div>
              </div>
              <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 8px",fontSize:11}} onClick={()=>deleteActividad(a.id)}>ðŸ—‘</button>
            </div>
          );
        })}
      </div>
      ...
      <div style={{...S.card,borderLeft:"5px solid #4CAF50",marginBottom:14}}>
        <div style={{fontWeight:800,fontSize:15,color:"#2E7D32",marginBottom:4}}>ðŸ™ Donativos</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginBottom:10}}>
          ...
          <input style={S.input} placeholder="â‚¬ Efectivo" value={donForm.efectivo} onChange={e=>setDonForm(f=>({...f,efectivo:e.target.value}))}/>
          <input style={S.input} placeholder="â‚¬ TPV" value={donForm.tpv} onChange={e=>setDonForm(f=>({...f,tpv:e.target.value}))}/>
        ...
        <button style={{...S.btn("#4CAF50","#FFFFFF",true),padding:10,fontSize:13,marginBottom:10}} onClick={saveDonativo}>âž• AÃ±adir donativo</button>
          ...
          return(
            <div key={d.id} style={{marginTop:8,padding:"8px 10px",borderRadius:10,background:"#F3FFF5",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div style={{fontSize:12,color:"#2D1B4E"}}>
                ...
                <div style={{fontSize:11,color:"#7B6B9A"}}>{d.fecha} Â· Donante: {d.responsable||"â€”"}</div>
              </div>
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,fontSize:11}}>
                <span style={{fontWeight:700,color:"#2E7D32"}}>Efec.: â‚¬{ef.toFixed(2)} Â· TPV: â‚¬{tpv.toFixed(2)}</span>
                <span style={{fontWeight:800,color:"#2E7D32",fontSize:13}}>Total: â‚¬{total.toFixed(2)}</span>
              </div>
              <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 8px",fontSize:11}} onClick={()=>deleteDonativo(d.id)}>ðŸ—‘</button>
            </div>
          );
        })}
      </div>
      ...
      <div style={{...S.card,borderLeft:"5px solid #F5C842"}}>
        <div style={{fontWeight:800,fontSize:15,color:"#B69100",marginBottom:4}}>ðŸ“ Votos</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginBottom:10}}>
          ...
          <input style={S.input} placeholder="â‚¬ Efectivo" value={votoForm.efectivo} onChange={e=>setVotoForm(f=>({...f,efectivo:e.target.value}))}/>
          <input style={S.input} placeholder="â‚¬ TPV" value={votoForm.tpv} onChange={e=>setVotoForm(f=>({...f,tpv:e.target.value}))}/>
        ...
        <button style={{...S.btn("#F5C842","#3D1B6B",true),padding:10,fontSize:13,marginBottom:10}} onClick={saveVoto}>âž• AÃ±adir voto</button>
          ...
          return(
            <div key={d.id} style={{marginTop:8,padding:"8px 10px",borderRadius:10,background:"#FFF9E6",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div style={{fontSize:12,color:"#2D1B4E"}}>
                ...
                <div style={{fontSize:11,color:"#7B6B9A"}}>{d.fecha} Â· Responsable: {d.responsable?displayMaestroNombre(d.responsable):"â€”"}</div>
              </div>
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2,fontSize:11}}>
                <span style={{fontWeight:700,color:"#2E7D32"}}>Efec.: â‚¬{ef.toFixed(2)} Â· TPV: â‚¬{tpv.toFixed(2)}</span>
                <span style={{fontWeight:800,color:"#2E7D32",fontSize:13}}>Total: â‚¬{total.toFixed(2)}</span>
...
function AdminApp({data,onUpdateData,onLogout,teacherPasswords,onUpdatePasswords,onRefreshData}){
  ...
  const tabs=[
    ...
    {id:"calificaciones",label:"Calif.",icon:"ðŸ“"},
    {id:"evaluaciones",label:"Evaluac.",icon:"â­"},
    {id:"clases",label:"Clases",icon:"ðŸ§’"},
    {id:"alumnos",label:"Alumnos",icon:"ðŸ‘¦"},
    {id:"maestros",label:"Maestros",icon:"ðŸ‘©â€ðŸ«"},
    {id:"mas",label:"MÃ¡s",icon:"â˜°"},
  ];
  ...
  const handleMergeAlumnos=useCallback(async(idA,idB)=>{
    ...
    if(!aA||!aB)return;
    // Elegir como principal el que tenga mÃ¡s datos completos
    const scoreAlumno=a=>{
      let s=0;
  ...
  const repairAlumnosYCalifs=useCallback(async()=>{
    ...
    if(!changed){
      alert("No se encontraron datos adicionales para restaurar. Ya estÃ¡ todo sincronizado.");
      return;
    }
    await onUpdateData("alumnos",alumnosCopy);
    await onUpdateData("calificaciones",nuevasCalifs);
    alert("Datos de alumnos y calificaciones reparados. Si no ves los cambios, recarga la pÃ¡gina.");
  ...
  const saveAdminPassword=async()=>{
    ...
    if(!adminPwForm.current||adminPwForm.current!==currentStored){
      setAdminPwForm(f=>({...f,error:"ContraseÃ±a actual incorrecta",ok:false}));
      return;
    }
    if(adminPwForm.new1.length<4){
      setAdminPwForm(f=>({...f,error:"La nueva contraseÃ±a debe tener al menos 4 caracteres",ok:false}));
      return;
    }
    if(adminPwForm.new1!==adminPwForm.new2){
      setAdminPwForm(f=>({...f,error:"Las contraseÃ±as nuevas no coinciden",ok:false}));
      return;
    }
    const nextProfile={...(data.adminProfile||{}),adminPassword:adminPwForm.new1};
    const ok=await onUpdateData("adminProfile",nextProfile);
    setAdminPwForm(f=>ok?{current:"",master:"",new1:"",new2:"",error:"",ok:true}:{...f,error:"No se pudo guardar. Revisa la conexiÃ³n.",ok:false});
  };
  ...
  const resetTeacherPassword=async()=>{
    ...
    if(resetForm.newPw.length<4){
      setResetForm(f=>({...f,error:"La nueva contraseÃ±a debe tener al menos 4 caracteres",ok:false}));
      return;
    }
    // ContraseÃ±a maestra: ADMIN_PASSWORD
    if(resetForm.master!==ADMIN_PASSWORD){
      setResetForm(f=>({...f,error:"ContraseÃ±a maestra incorrecta",ok:false}));
      return;
    }
  ...
  const handleRestoreBackupFile=async(e)=>{
    ...
    try{
      ...
      if(!window.confirm("Se reemplazarÃ¡n los datos actuales por los de la copia. Esta acciÃ³n no se puede deshacer. Â¿Continuar?")){
        return;
      }
      setRestoreBusy(true);
      ...
      alert("Copia restaurada correctamente. Se recomienda recargar la pÃ¡gina para ver todos los cambios.");
    }catch(err){
      console.error("Error restaurando copia:",err);
      alert("No se pudo restaurar la copia de seguridad. Verifica el archivo e intÃ©ntalo de nuevo.");
    }finally{
  ...
  return(
    <div style={{background:"#F5F0FF",minHeight:"100dvh",paddingBottom:70}}>
      <div style={{background:"linear-gradient(135deg,#3D1B6B,#5B2D8E)",padding:"0.75rem 1rem",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 2px 12px rgba(0,0,0,0.15)",position:"sticky",top:0,zIndex:100}}>
        <LogoImg height={38} onClick={()=>onRefreshData?.()} title="Actualizar datos"/>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{background:"#F5C84233",color:"#F5C842",borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:800}}>ðŸ‘‘ ADMIN</span>
          <button style={{background:"rgba(255,255,255,0.18)",border:"none",borderRadius:10,padding:"8px 12px",color:"#FFFFFF",fontWeight:700,fontSize:13,cursor:"pointer"}} onClick={onLogout}>ðŸšª Salir</button>
        </div>
      </div>
      <div>
        ...
        {activeTab==="mas"&&(
          <div style={{padding:"1rem 1rem 0"}}>
            <div style={{display:"flex",gap:8,overflowX:"auto",marginBottom:16,paddingBottom:4}}>
              {[["familias","ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ Familias"],["eventos","ðŸ“† Eventos"],["cumpleanos","ðŸŽ‚ Cumple."],["finanzas","ðŸ’° Finanzas"],["informes","ðŸ“„ Informes"],["perfilAdmin","ðŸ‘‘ Admin"]].map(([id,label])=>(
                <button key={id} style={{...S.btn(masTab===id?"#5B2D8E":"#F5F0FF",masTab===id?"#FFFFFF":"#2D1B4E"),padding:"8px 14px",fontSize:13,flexShrink:0,borderRadius:20,whiteSpace:"nowrap"}} onClick={()=>setMasTab(id)}>{label}</button>
              ))}
            </div>
            ...
            {masTab==="perfilAdmin"&&(
              <div style={{paddingBottom:20}}>
                <h2 style={S.title}>ðŸ‘‘ Admin</h2>
                <div style={{...S.card,marginBottom:14}}>
                  <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,margin:"0 0 8px"}}>ðŸ—‚ï¸ Copia de seguridad</h3>
                ...
                <div style={{...S.card,marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
                    ...
                    <div>
                      <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,margin:"0 0 4px"}}>ðŸ’¼ Datos visibles del admin</h3>
                      <p style={{fontSize:11,color:"#7B6B9A",margin:0}}>AquÃ­ puedes configurar cÃ³mo se verÃ¡ el administrador en los informes.</p>
                    </div>
                  </div>
                <div style={{...S.card,marginBottom:14}}>
                  <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,margin:"0 0 8px"}}>ðŸ©º Reparar datos de alumnos y calificaciones</h3>
                  <p style={{fontSize:12,color:"#7B6B9A",margin:"0 0 10px"}}>
                    Si despuÃ©s de fusionar alumnos notas que alguna foto u observaciones de clases desaparecieron, puedes intentar restaurarlas desde los datos guardados en Clases, Familias y Calificaciones.
                    No se crearÃ¡n alumnos nuevos; solo se completarÃ¡n datos faltantes y se reasignarÃ¡n calificaciones a los alumnos actuales.
                  </p>
                  <button
                    style={{...S.btn("#4BBCE0","#FFFFFF",true),padding:"8px 14px",fontSize:13,borderRadius:10}}
                    onClick={repairAlumnosYCalifs}
                  >
                    ðŸ”§ Reparar alumnos y calificaciones
                  </button>
                </div>
                  ...
                  />
                  <p style={{fontSize:11,color:"#7B6B9A",margin:0}}>Solo se usa para mostrar en informes u otras pantallas de administraciÃ³n.</p>
                </div>
                <div style={{...S.card,marginBottom:14}}>
                  <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,marginBottom:10}}>ðŸ” Cambiar contraseÃ±a de administrador</h3>
                  <p style={{fontSize:11,color:"#7B6B9A",marginBottom:6}}>Esta contraseÃ±a se usarÃ¡ para entrar como administrador (ademÃ¡s de la contraseÃ±a maestra fija).</p>
                  <p style={{fontSize:11,color:"#9E9E9E",marginBottom:10,fontStyle:"italic"}}>ContraseÃ±a maestra (fija): <strong>{ADMIN_PASSWORD}</strong></p>
                  <label style={S.label}>ContraseÃ±a actual</label>
                  ...
                  <label style={S.label}>Nueva contraseÃ±a</label>
                  ...
                  <label style={S.label}>Confirmar nueva contraseÃ±a</label>
                  ...
                  {adminPwForm.ok&&<div style={{color:"#4CAF50",fontSize:12,fontWeight:700,marginBottom:8}}>ContraseÃ±a de admin actualizada.</div>}
                  <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:12}} onClick={saveAdminPassword}>ðŸ’¾ Guardar contraseÃ±a admin</button>
                </div>
                <div style={S.card}>
                  <h3 style={{color:"#5B2D8E",fontWeight:800,fontSize:15,marginBottom:10}}>ðŸ”‘ Restablecer contraseÃ±a de maestro/auxiliar</h3>
                  <p style={{fontSize:11,color:"#7B6B9A",marginBottom:10}}>Usa la <strong>contraseÃ±a maestra de administrador</strong> para poder cambiar la contraseÃ±a de cualquier maestro o auxiliar.</p>
                  <label style={S.label}>Seleccionar maestro/auxiliar</label>
                  <select
                    style={{...S.input,marginBottom:10}}
                  ...
                  />
                  <label style={S.label}>ContraseÃ±a maestra (solo admin)</label>
                  <input
                    type="password"
                    style={{...S.input,marginBottom:10}}
                    value={resetForm.master}
                    onChange={e=>setResetForm(f=>({...f,master:e.target.value,error:"",ok:false}))}
                    placeholder="ContraseÃ±a maestra de administrador"
                  />
                  {resetForm.error&&<div style={{color:"#EF5350",fontSize:12,fontWeight:700,marginBottom:8}}>{resetForm.error}</div>}
                  {resetForm.ok&&<div style={{color:"#4CAF50",fontSize:12,fontWeight:700,marginBottom:8}}>ContraseÃ±a actualizada para el maestro seleccionado.</div>}
                  <button style={{...S.btn("#E84F9B","#FFFFFF",true),padding:12}} onClick={resetTeacherPassword}>ðŸ”‘ Restablecer contraseÃ±a</button>
                </div>
              </div>
            )}
          </div>
