function App(){
  ...
  useEffect(()=>{
    ...
    const unsub=subscribeData((key,val)=>{
      if(key==="teacherPasswords")setTeacherPasswords(val||{});
        ...
        // No perder fotos: si el servidor devuelve alumnos sin foto, conservar la que tenÃ­amos en estado
        setData(d=>{
  ...
  // OpciÃ³n de instalar web app: Android (beforeinstallprompt) e iOS (instrucciones AÃ±adir a pantalla de inicio)
  useEffect(()=>{
      ...
      <div style={{width:"100%",maxWidth:"100vw",overflowX:"hidden",minHeight:"100dvh",touchAction:"pan-y pinch-zoom",position:"relative"}}>
        ...
        {showInstallBanner&&installPrompt&&!installDone&&(
          <div style={{position:"fixed",bottom:0,left:0,right:0,background:"linear-gradient(135deg,#3D1B6B,#5B2D8E)",color:"#FFFFFF",padding:"12px 16px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 -4px 20px rgba(0,0,0,0.2)",zIndex:9999}}>
            <div style={{flex:1,fontSize:14,fontWeight:700}}>ðŸ“² Instalar Escuela Dominical en tu dispositivo</div>
            ...
            <button onClick={()=>setShowInstallBanner(false)} style={{background:"transparent",color:"rgba(255,255,255,0.9)",border:"none",padding:8,fontSize:18,cursor:"pointer",lineHeight:1}} aria-label="Cerrar">Ã—</button>
          </div>
        )}
        {showIosInstallBanner&&!isIosStandalone&&(
          <div style={{position:"fixed",bottom:0,left:0,right:0,background:"linear-gradient(135deg,#3D1B6B,#5B2D8E)",color:"#FFFFFF",padding:"14px 16px",boxShadow:"0 -4px 20px rgba(0,0,0,0.2)",zIndex:9999}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
              <div style={{flex:1}}>
                <div style={{fontSize:15,fontWeight:800,marginBottom:6}}>ðŸ“² Instalar en iPhone/iPad</div>
                <div style={{fontSize:13,opacity:0.95,lineHeight:1.4}}>Pulsa el botÃ³n <strong>Compartir</strong> (â–¡â†‘) abajo en Safari y luego <strong>Â«AÃ±adir a pantalla de inicioÂ»</strong>. La app quedarÃ¡ como un icono en tu pantalla.</div>
              </div>
              <button onClick={()=>setShowIosInstallBanner(false)} style={{background:"transparent",color:"rgba(255,255,255,0.9)",border:"none",padding:6,fontSize:20,cursor:"pointer",lineHeight:1,flexShrink:0}} aria-label="Cerrar">Ã—</button>
            </div>
          </div>
        )}
      </div>
