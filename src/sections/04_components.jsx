      ...
      <div style={{background:"rgba(255,255,255,0.97)",borderRadius:28,padding:"1.5rem 1.25rem",width:"100%",maxWidth:400,boxShadow:"0 24px 80px rgba(0,0,0,0.35)",position:"relative",zIndex:1}}>
        ...
        {mode==="select"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <h3 style={{textAlign:"center",color:"#5B2D8E",fontWeight:800,margin:"0 0 4px",fontSize:18}}>¿Cómo deseas entrar?</h3>
...
function AdminDashboard({data,onUpdateData,onGoToTab}){
  ...
  const deletePeticion=()=>{if(peticionEditId&&!confirmDelete("¿Eliminar esta petición de oración?"))return;if(peticionEditId){onUpdateData("peticiones",(peticiones||[]).filter(p=>p.id!==peticionEditId));setPeticionModal(false);}};
  return(
    <div style={{paddingBottom:10}}>
      ...
      <div style={{padding:"0 1rem 6.25rem"}}>
        ...
        <div style={S.card}>
          ...
          {upcomingEvents.length===0&&<div style={{color:"#7B6B9A"}}>Sin eventos próximos.</div>}
        ...
        <div style={{...S.card,borderLeft:"5px solid #EF5350"}}>
          <h3 style={{color:"#EF5350",fontWeight:800,fontSize:15,marginBottom:10}}>⚠️ Certificados Pendientes</h3>
    ...
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      {[...filtered].sort((a,b)=>sortKeyFirstName(a.nombre).localeCompare(sortKeyFirstName(b.nombre),"es")).map(m=>(
        <div key={m.id} style={{...S.card,display:"flex",alignItems:"center",gap:12}}>
          ...
          <button style={{...S.btn("#4BBCE0"),padding:"8px 12px",fontSize:13}} onClick={()=>openEdit(m)}>✏️</button>
        </div>
      ))}
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
        <button style={{...S.btn("#F5C842","#3D1B6B",true),padding:"12px",borderRadius:14,fontSize:14,flex:1}} onClick={()=>setUnavailModal(true)}>⚠️ Ausencias {unavailable.length>0?`(${unavailable.length})`:""}</button>
        <button style={{...S.btn("#4BBCE0","#FFFFFF",true),padding:"12px",borderRadius:14,fontSize:14,flex:1}} onClick={()=>setStatsModal(true)}>📊 Estadísticas</button>
      </div>
              ...
              <div key={e.id} style={{background:(CLASE_COLORS[e.grupo]||"#5B2D8E")+"12",border:`1.5px solid ${CLASE_COLORS[e.grupo]||"#5B2D8E"}44`,borderRadius:14,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",gap:8}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontWeight:800,fontSize:15,marginTop:6}}>{e.leccion||"(sin lección)"}</div>
                  ...
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    ...
                    <button
                      ...
                      onClick={()=>{if(confirmDelete("¿Eliminar esta sesión del cronograma?"))onUpdate(cronograma.filter(c=>c.id!==e.id));}}
                      title="Eliminar sesión (también si no hay clase)"
                    >🗑</button>
                  </div>
                </div>
                {[["maestro","MAESTRO","🎓","#5B2D8E","asistioMaestro","fallaMaestro"],["auxiliar","AUXILIAR","🤝","#E84F9B","asistioAuxiliar","fallaAuxiliar"]].filter(([role])=>!(e.grupo==="ADOLESCENTES"&&role==="auxiliar")).map(([role,label,icon,color,asistioKey,fallaKey])=>{
                  ...
                  <div key={role} style={{marginTop:8,background:"rgba(255,255,255,0.85)",borderRadius:10,padding:"10px 12px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div style={{flex:1,fontWeight:700,fontSize:13,color:nombreRole&&unavailable.includes(nombreRole)?"#EF5350":"#2D1B4E"}}>
                        {nombreRole?(unavailable.includes(nombreRole)?"⚠️ ":icon+" ")+displayMaestroNombre(nombreRole):<span style={{color:"#7B6B9A",fontStyle:"italic"}}>Sin asignar</span>}
                      </div>
                      {puedeRegistrarAsistencia&&(
                        <div style={{display:"flex",gap:4}}>
                          <button style={{...S.btn(asistio===true?"#4CAF50":"#F5F0FF",asistio===true?"#FFF":"#2D1B4E"),padding:"4px 8px",fontSize:11}} onClick={()=>onUpdate(cronograma.map(c=>c.id===e.id?{...c,[asistioKey]:true}:c))} title="Asistió">✓</button>
                          <button style={{...S.btn(asistio===false?"#EF5350":"#F5F0FF",asistio===false?"#FFF":"#2D1B4E"),padding:"4px 8px",fontSize:11}} onClick={()=>onUpdate(cronograma.map(c=>c.id===e.id?{...c,[asistioKey]:false}:c))} title="No asistió">✕</button>
                        </div>
                      )}
                      {nombreRole&&<button style={{...S.btn("#EF5350"),padding:"6px 10px",fontSize:11}} onClick={()=>{if(!window.confirm("¿Registrar falla a "+displayMaestroNombre(nombreRole)+" para esta sesión?\n\nSe le restará 1 punto de cumplimiento, se quitará del horario y se abrirá la selección de sustituto."))return;onUpdate(cronograma.map(c=>c.id===e.id?{...c,[fallaKey]:nombreRole,[role]:"",[asistioKey]:undefined}:c));setSubModal({entryId:e.id,role,grupo:e.grupo});setActiveDate(d);}} title="Falla: quita al maestro/auxiliar y resta cumplimiento; elige sustituto">Falla</button>}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:380,overflowY:"auto"}}>
          {maestros.map(m=>(
            <div key={m.id} onClick={()=>setUnavailable(u=>u.includes(m.nombre)?u.filter(n=>n!==m.nombre):[...u,m.nombre])}
              ...
              <div style={{flex:1}}><div style={{fontWeight:700}}>{displayMaestroNombre(m.nombre)}</div><div style={{fontSize:12,color:"#7B6B9A"}}>{m.cargo} · {m.clase}</div></div>
      ...
      <Modal open={statsModal} onClose={()=>setStatsModal(false)} title="📊 Estadísticas de Participación">
            ...
            return(
              <div key={m.id} style={{...S.card,padding:"12px 14px",marginBottom:10}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  ...
                  {st.noClass>0&&<span style={{...S.badge("#EF5350"),fontSize:11}}>⚠️ {st.noClass} sin clase</span>}
                </div>
                    ...
                    <div key={m.id} onClick={()=>{onUpdate(cronograma.map(c=>c.id===subModal.entryId?{...c,[subModal.role]:m.nombre}:c));setSubModal(null);}}
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:6,marginTop:3,flexWrap:"wrap"}}>
                          ...
                          {warn&&st.total>8&&<span style={{...S.badge("#F5C842"),fontSize:10}}>⚠️ Muchas clases</span>}
                        </div>
                      </div>
      ...
      <Modal open={addModal} onClose={()=>setAddModal(false)} title="📅 Nueva Clase">
        ...
        <div style={{background:"linear-gradient(135deg,#4BBCE015,#4BBCE025)",border:"1.5px solid #4BBCE055",borderRadius:12,padding:"10px 14px",marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {form.maestro&&<span style={{...S.badge("#5B2D8E"),fontSize:11}}>🎓 {displayMaestroNombre(form.maestro)}</span>}
            {form.auxiliar&&<span style={{...S.badge("#E84F9B"),fontSize:11}}>🤝 {displayMaestroNombre(form.auxiliar)}</span>}
        ...
        {editEntry&&(
          <div>
            <div style={{background:"#F5F0FF",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
              <div style={{fontWeight:800,color:"#5B2D8E",fontSize:15}}>{editEntry.leccion||"(sin lección)"}</div>
            ...
            {[["Lección","leccion"],["Tema","tema"],["Fecha","fecha"]].map(([l,k])=>(
            ...
            <p style={{fontSize:12,color:"#7B6B9A",marginTop:12,marginBottom:4}}>Puedes eliminar esta sesión aunque no tenga clase asignada (ej. NO HAY CLASE).</p>
            <button style={{...S.btn("#FFF0F0","#EF5350"),padding:12,marginTop:4,border:"1.5px solid #EF535044"}} onClick={()=>{if(confirmDelete("¿Eliminar esta sesión del cronograma?")){onUpdate(cronograma.filter(c=>c.id!==editEntryId));setEditEntryId(null);}}}>🗑 Eliminar Sesión</button>
          </div>
        )}
      </Modal>
    </div>
    ...
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editIdx!=null?"Editar Alumno":`Agregar a ${activeClase}`}>
        ...
        {editIdx==null?(<>
          ...
          <input style={{...S.input,marginBottom:12}} placeholder="Ej: José Luis" value={form.nombre} onChange={e=>setForm(f=>({...f,nombre:e.target.value}))}/>
          <label style={S.label}>Apellidos</label>
          <input style={{...S.input,marginBottom:12}} placeholder="Ej: Mogollón Muñoz" value={form.apellido} onChange={e=>setForm(f=>({...f,apellido:e.target.value}))}/>
        </>):(
      ...
      <Modal open={mgrModal} onClose={()=>{setMgrModal(false);setClaseForm(null);}} title="⚙️ Gestionar Clases">
        {!claseForm?(
          <div>
            <div style={{marginBottom:14,fontSize:13,color:"#7B6B9A"}}>Edita el nombre o color de cada clase, o añade una nueva.</div>
            {cfg.map(cl=>(
              <div key={cl.key} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid #EEE8FF"}}>
                ...
                <button style={{...S.btn("#4BBCE0"),padding:"6px 11px",fontSize:12}} onClick={()=>setClaseForm({...cl,isNew:false})}>✏️ Editar</button>
...
function FamiliasPanel({familias,onUpdate,clases={},onUpdateClases=()=>{},teacherMode=false,readOnly=false}){
  ...
  const eliminarEncargo=(familyKey,encargoId)=>{
    if(!confirmDelete("¿Eliminar este encargo del historial de la familia?"))return;
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
        ...
        return(
          <div key={key} style={{...S.card,borderLeft:"5px solid #5B2D8E"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:800,color:"#5B2D8E",fontSize:15}}>👨‍👩‍👧 {members[0].familia||displayNameAlumno(members[0])||key}</div>
                <div style={{fontSize:11,color:"#7B6B9A"}}>{members.length} alumno{members.length!==1?"s":""}</div>
              </div>
              <button style={{...S.btn("#F5C842","#2D1B4E"),padding:"6px 10px",fontSize:11,flexShrink:0}} onClick={()=>abrirEncargo(key)} title="Asignar un servicio a esta familia">📝 Encargar servicio</button>
            </div>
            ...
            <div style={{marginBottom:12}}>
              ...
              >
                <span>📝 Historial de encargos de servicio {encargos.length>0?`(${encargos.length})`:""}</span>
                <span style={{fontSize:18}}>{openEncargosKey===key?"▼":"▶"}</span>
              </button>
              {openEncargosKey===key&&(
                <div style={{marginTop:8,padding:"12px 14px",background:"#FFFEF5",borderRadius:10,border:"1px solid #F5C84233"}}>
                  {encargos.length===0?(
                    <div style={{fontSize:13,color:"#7B6B9A",fontStyle:"italic"}}>Aún no hay encargos. Usa «Encargar servicio» para asignar uno.</div>
                  ):(
                    encargos.slice().sort((a,b)=>(b.fecha||"").localeCompare(a.fecha||"")).map(e=>(
                        <div key={e.id} style={{padding:"10px 0",borderBottom:"1px solid #F5C84233"}}>
                          ...
                          <div style={{fontSize:11,color:"#7B6B9A",marginBottom:8}}>📅 {e.fecha||"—"}</div>
                          <div style={{fontSize:11,color:"#7B6B9A",marginBottom:4}}>Estado: {e.estado==="CUMPLIDO"?"✅ Cumplió":e.estado==="NO_CUMPLIO"?"❌ No cumplió":"⏳ Pendiente"}</div>
                          <div style={{display:"flex",flexWrap:"wrap",gap:8,alignItems:"center",marginBottom:6}}>
                            <span style={{fontSize:11,color:"#7B6B9A"}}>¿Cumplió?</span>
                            <button style={{...S.btn(e.estado==="CUMPLIDO"?"#4CAF50":"#F5F0FF",e.estado==="CUMPLIDO"?"#FFF":"#2D1B4E"),padding:"4px 10px",fontSize:11}} onClick={()=>updateEncargoInline(key,e.id,{...e,estado:"CUMPLIDO"})}>Cumplió</button>
                            <button style={{...S.btn(e.estado==="NO_CUMPLIO"?"#EF5350":"#F5F0FF",e.estado==="NO_CUMPLIO"?"#FFF":"#2D1B4E"),padding:"4px 10px",fontSize:11}} onClick={()=>updateEncargoInline(key,e.id,{...e,estado:"NO_CUMPLIO"})}>No cumplió</button>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",marginBottom:6}}>
                            <span style={{fontSize:11,color:"#7B6B9A"}}>Calificación:</span>
                            {[1,2,3,4,5].map(s=>(
                              <button key={s} type="button" style={{padding:2,border:"none",background:"transparent",cursor:"pointer",fontSize:18}} onClick={()=>updateEncargoInline(key,e.id,{...e,calificacion:s})} title={`${s} estrella(s)`}>
                                <span style={{color:(e.calificacion!=null&&e.calificacion>=s)?"#F5C842":"#e0e0e0"}}>★</span>
                              </button>
                            ))}
                            {e.calificacion!=null&&<span style={{fontSize:11,color:"#7B6B9A"}}>{e.calificacion}/5</span>}
                          </div>
                          <div style={{display:"flex",gap:6}}>
                            <button style={{...S.btn("#F5C842","#2D1B4E"),padding:"4px 8px",fontSize:11}} onClick={()=>abrirEncargo(key,e)}>✏️ Editar / Nota</button>
                            <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 8px",fontSize:11}} onClick={()=>eliminarEncargo(key,e.id)}>🗑 Eliminar</button>
                          </div>
                        </div>
                    ))
                  )}
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editId?"Editar Alumno":"Nuevo Miembro"}>
        ...
        {editId&&!teacherMode&&<button style={{...S.btn("#FFF0F0","#EF5350"),padding:12,marginTop:10,border:"1.5px solid #EF535044"}} onClick={()=>{if(confirmDelete("¿Eliminar a "+displayNameAlumno(form)+"?")){onUpdate(familias.filter(f=>f.id!==editId));setModal(false);}}}>🗑 Eliminar Miembro</button>}
      </Modal>
      <Modal open={encargoModal} onClose={()=>{setEncargoModal(false);setEncargoEditId(null);}} title={encargoEditId?"Revisar / Calificar servicio":"Encargar servicio a la familia"}>
        <div style={{fontSize:12,color:"#7B6B9A",marginBottom:10}}>
          {encargoEditId?"Puedes calificar cuando haya pasado la fecha de la tarea. Queda en el historial de calificación de la familia.":"Solo servicio y fecha. La calificación se hará después, cuando pase la fecha de la tarea."}
        </div>
        ...
        {encargoEditId&&(()=>{
          ...
          const yaPasó=!(encargoForm.fecha&&encargoForm.fecha>hoyStr);
          if(!yaPasó)return(
            <div style={{background:"#FFF8E0",borderRadius:10,padding:"12px 14px",marginBottom:14,fontSize:13,color:"#7B5A00"}}>
              Podrás calificar cuando pase la fecha de la tarea ({encargoForm.fecha}).
            </div>
          );
          return(
            <>
              ...
              <textarea style={{...S.input,minHeight:70,resize:"vertical",marginBottom:14}} value={encargoForm.nota} onChange={e=>setEncargoForm(f=>({...f,nota:e.target.value}))} placeholder="Comentarios sobre cómo cumplió la familia."/>
            </>
          );
        })()}
        {!encargoEditId&&(
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:14}} onClick={guardarEncargo}>{encargoEditId?"💾 Guardar calificación":"💾 Guardar encargo"}</button>
          {encargoEditId&&encargoFamilyKey&&<button style={{...S.btn("#FFF0F0","#EF5350"),padding:14,border:"1.5px solid #EF535044"}} onClick={()=>eliminarEncargo(encargoFamilyKey,encargoEditId)}>🗑 Eliminar encargo</button>}
        </div>
      </Modal>
    </div>
  );
...
function AlumnosPanel({
  ...
  // Alumnos que aparecen en calificaciones pero no en la lista (solo se usa para información en paneles; la recuperación automática se hace al cargar datos)
  ...
  const deleteAlumno=(a)=>{
    if(!confirmDelete("¿Eliminar a "+displayNameAlumno(a)+"?"))return;
    ...
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      {onMergeAlumnos&&(
        <div style={{background:"#E0F2FE",border:"1.5px solid #38BDF8",borderRadius:14,padding:"12px 14px",marginBottom:16}}>
          <div style={{fontWeight:800,fontSize:13,color:"#0F172A",marginBottom:6}}>🔗 Fusionar alumnos duplicados</div>
          <div style={{fontSize:12,color:"#0F172A",marginBottom:8}}>
            Si detectas que un alumno está repetido, selecciona ambos registros y marca que son la misma persona. Se unificarán los datos y calificaciones en un solo alumno.
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8,alignItems:"center"}}>
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              ...
              >
                ...
                {sorted.map(a=>(
                  <option key={a.id} value={a.id}>{displayNameAlumno(a)} — {normalizarClase(a.clase)}</option>
                ))}
              </select>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
            ...
            <button
              ...
              onClick={()=>{
                ...
                if(!aA||!aB)return;
                if(!window.confirm("Vas a fusionar a '"+displayNameAlumno(aA)+"' y '"+displayNameAlumno(aB)+"'.\n\nEl sistema conservará el alumno con más datos completos y unirá todas las calificaciones de ambos.\n\nEsta acción no se puede deshacer fácilmente. ¿Continuar?"))return;
            ...
            >
              🔗 Marcar como mismo alumno y fusionar
            </button>
          </div>
        ...
        listado.map(a=>{
          ...
          return(
            <div key={a.id} style={{...S.card,display:"flex",alignItems:"center",gap:12,borderLeft:"4px solid "+color}}>
              ...
              <button style={{...S.btn("#FFF0F0","#EF5350"),padding:"8px 12px"}} onClick={()=>{if(!confirmDelete("¿Eliminar a "+displayNameAlumno(a)+"?"))return;deleteAlumno(a);}} title="Eliminar">🗑</button>
            </div>
          );
        })
      )}
...
function CalifAdminPanel({calificaciones,clases,criterios,onUpdate,cronograma=[],meriendas=[]}){
  ...
  const resetCalifSesionAdmin=(sesionId)=>{
    if(!confirmDelete("¿Resetear todas las calificaciones de esta sesión para la clase "+activeClase+"? Los alumnos quedarán como ausentes."))return;
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        ...
        <button style={{...S.btn(viewMode==="sesiones"?"#4BBCE0":"#F5F0FF",viewMode==="sesiones"?"#FFFFFF":"#2D1B4E"),flex:1,padding:"10px",borderRadius:12,fontSize:13}} onClick={()=>setViewMode("sesiones")}>📋 Por Sesión</button>
      </div>
                ...
                {allObs.map((obs,i)=>(
                  <div key={i} style={{marginBottom:i<allObs.length-1?12:0,paddingBottom:i<allObs.length-1?12:0,borderBottom:i<allObs.length-1?"1px dashed #F5C84280":"none"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                      ...
                      {obs.quien&&<span style={{fontSize:11,color:"#7B6B9A"}}>· por {displayMaestroNombre(obs.quien)}</span>}
                    </div>
      ...
      {viewMode==="sesiones"&&(
        <div>
          {sesiones.length===0&&<div style={{color:"#7B6B9A",textAlign:"center",padding:30}}>Sin sesiones calificadas aún.</div>}
          {sesiones.map(ses=>{
            ...
            let gastoLabel="";
            if(m?.merienda)gastoLabel+="🍎 Merienda"+(m.meriendaDonativo?" (donativo)":"");
            if(m?.merienda&&m?.trabajoManual)gastoLabel+=" + ";
            if(m?.trabajoManual)gastoLabel+="✂️ T.Manual"+(m.trabajoManualDonativo?" (donativo)":"");
            ...
            return(
              <div key={ses.sesionId} style={{...S.card,borderLeft:`5px solid ${CLASE_COLORS[activeClase]||"#5B2D8E"}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,flexWrap:"wrap",gap:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    {sesCalifs.length>0&&(
                      <button style={{...S.btn("#FFF0E6","#C62828"),padding:"5px 10px",fontSize:11}} onClick={()=>resetCalifSesionAdmin(ses.sesionId)} title="Borrar todas las calificaciones de esta sesión">🔄 Resetear</button>
                    )}
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                      {sesCalifs.map((e,i)=>{
                        ...
                        return <div key={i} style={{background:a?scoreColor(a)+"20":"#F5F0FF",borderRadius:8,padding:"4px 10px",fontSize:12,border:`1.5px solid ${a?scoreColor(a)+"44":"#DDD0F0"}`,display:"inline-flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                          ...
                          <span style={{fontWeight:700}}>{shortDisplayName(e.alumno)}</span>{a&&<span style={{color:scoreColor(a),fontWeight:800}}> {a}</span>}{e.quienCalifico&&<span style={{fontSize:10,color:"#7B6B9A"}}>· por {displayMaestroNombre(e.quienCalifico)}</span>}
                        </div>;
                      })}
                    </div>
                      ...
                      <div style={{marginTop:10,background:"#F5C84210",border:"1.5px solid #F5C84244",borderRadius:10,padding:"10px 12px"}}>
                        ...
                        {sesCalifs.filter(e=>e.observacion).map((e,i)=>{
                          ...
                          return <div key={i} style={{fontSize:12,color:"#2D1B4E",marginBottom:4,lineHeight:1.5,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                            ...
                            <span><strong>{shortDisplayName(e.alumno)}:</strong> {e.observacion}{e.quienObservacion?<span style={{fontSize:11,color:"#7B6B9A"}}> — por {displayMaestroNombre(e.quienObservacion)}</span>:""}</span>
                          </div>;
                        })}
                      </div>
                    )}
...
// Formato corto para registro de quién calificó / observó (calificaciones de alumnos)
...
function EvaluacionesPanelUnificado({evaluaciones,onUpdate,videos,onUpdateVideos,cronograma,onUpdateCronograma,maestros}){
  ...
  // Orden: primero pendientes por calificar (sin evaluación), ordenados por fecha de sesión (quien dio clase más reciente primero); luego el resto alfabéticamente
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#4BBCE015,#5B2D8E15)",border:"1.5px solid #4BBCE055",borderRadius:14,padding:"12px 14px",marginBottom:16,fontSize:13,color:"#2D1B4E"}}>
        ...
        <div style={{color:"#5B2D8E",fontSize:12}}>Quienes faltan por calificar aparecen primero (por fecha de su última clase); el resto en orden alfabético. El icono <strong>🎬</strong> indica videos pendientes de calificar.</div>
      </div>
          ...
          <div key={nombre} style={{...S.card,borderLeft:`5px solid ${ev?color:"#DDD0F0"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                ...
                {!ev&&<div style={{fontSize:11,color:"#F5A623",fontWeight:700,marginTop:2}}>⏳ Pendiente por calificar</div>}
                {vAvg!=null&&<div style={{fontSize:11,color:"#4BBCE0",fontWeight:700}}>🎬 Video: {vAvg.toFixed(1)}/5 · incl. en promedio</div>}
              </div>
              {tienePendientes&&(
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {EVAL_KEYS.map((k,j)=>(
                  <div key={k} style={{background:k==="cumplimientoClases"?"linear-gradient(135deg,#5B2D8E15,#5B2D8E25)":"#F5F0FF",borderRadius:10,padding:"8px 10px",border:k==="cumplimientoClases"?"1.5px solid #5B2D8E44":"none"}}>
                    ...
                    <div style={{display:"flex",gap:1}}>{[1,2,3,4,5].map(s=><span key={s} style={{color:s<=(evDisplay[k]||0)?"#F5C842":"#e0e0e0",fontSize:16}}>★</span>)}</div>
                  </div>
                ))}
              </div>
            )}
      ...
      <Modal open={editModal} onClose={()=>setEditModal(false)} title={editForm?(editIdx===-1?`Nueva evaluación: ${displayMaestroNombre(editForm.nombre)}`:`Editar: ${displayMaestroNombre(editForm.nombre)}`):""}>
        ...
        {editForm&&(
          <div style={{marginBottom:16,background:"linear-gradient(135deg,#5B2D8E12,#5B2D8E22)",borderRadius:12,padding:"12px 14px",border:"1.5px solid #5B2D8E44"}}>
            <label style={{...S.label,color:"#5B2D8E",fontWeight:800}}>✅ Cumplimiento de Clases (automático)</label>
            <div style={{fontSize:13,color:"#7B6B9A",marginTop:4}}>Se calcula automáticamente según las clases asignadas: empieza en 5 estrellas. Cada falla registrada en Horario (botón Falla) resta 1 punto. Con una falla nunca podrá tener las 5 estrellas. Las fallas quedan registradas más abajo con fecha, lección y grupo.</div>
            <div style={{display:"flex",gap:4,marginTop:8,alignItems:"center"}}>
              {[1,2,3,4,5].map(s=>(
                <span key={s} style={{color:s<=(cumplimientoClasesFromCronograma(editForm.nombre,cronograma)??0)?"#F5C842":"#e0e0e0",fontSize:20}}>★</span>
              ))}
              <span style={{fontSize:12,color:"#5B2D8E",marginLeft:4}}>{(cumplimientoClasesFromCronograma(editForm.nombre,cronograma)??"—")+"/5"}</span>
            </div>
            {(()=>{
              ...
              const quitarFalla=(ent)=>{
                ...
                if(!window.confirm("¿Quitar esta falla del historial? Ya no se descontará cumplimiento para esta sesión."))return;
              ...
              return(
                <div style={{marginTop:12,paddingTop:10,borderTop:"1px solid #DDD0F0"}}>
                  <div style={{fontSize:11,fontWeight:800,color:"#EF5350",marginBottom:6}}>📋 Registro de fallas (fecha, lección, grupo) — Admin puede quitar si fue un error</div>
                  {fallasReg.map(e=>(
                    <div key={e.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:12,color:"#2D1B4E",marginBottom:6}}>
                      <span style={{flex:1}}>📅 {e.fecha} · {e.leccion||"—"} · {e.grupo||"—"}</span>
                      {onUpdateCronograma&&<button type="button" style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 10px",fontSize:11}} onClick={()=>quitarFalla(e)} title="Quitar esta falla del historial">🗑 Quitar falla</button>}
                    </div>
                  ))}
                </div>
              );
        ...
        {editForm&&(()=>{
          ...
          const eliminarVideoCalif=(v)=>{if(!confirmDelete("¿Eliminar esta calificación de video del historial? Se borrará el registro de esta sesión."))return;onUpdateVideos(videos.filter(vid=>!(vid.sesionId===v.sesionId&&vid.maestro===v.maestro)));};
          return(
            <div style={{marginBottom:16,background:"#4BBCE008",borderRadius:12,padding:"12px 14px",border:"1.5px solid #4BBCE044"}}>
              <label style={{...S.label,color:"#4BBCE0",fontWeight:800}}>🎬 Historial de calificaciones (videos)</label>
              <div style={{fontSize:11,color:"#7B6B9A",marginTop:4,marginBottom:8}}>Cada fila es una sesión con video calificado. Admin puede editar o eliminar si hubo error.</div>
              {conCalif.map(({ses,v,score})=>(
                <div key={ses.id} style={{display:"flex",alignItems:"center",gap:8,fontSize:12,marginBottom:8,flexWrap:"wrap"}}>
                  ...
                  <span style={{color:"#7B6B9A"}}>📅 {ses.fecha} · {ses.grupo}</span>
                  <span style={{fontWeight:800,color:score>=4?"#4CAF50":score>=2.5?"#F5A623":"#EF5350"}}>{score!=null?score.toFixed(1):"—"}/5</span>
                  <button type="button" style={{...S.btn("#4BBCE0","#FFF"),padding:"4px 10px",fontSize:11}} onClick={()=>openVideoEdit(ses)} title="Editar calificación">✏️ Editar</button>
                  <button type="button" style={{...S.btn("#FFF0F0","#EF5350"),padding:"4px 10px",fontSize:11}} onClick={()=>eliminarVideoCalif(v)} title="Eliminar del historial">🗑 Eliminar</button>
                </div>
              ))}
            </div>
          );
      ...
      <Modal open={!!videoModalMaestro} onClose={()=>{setVideoModalMaestro(null);setEditVideo(null);}} title={videoModalMaestro?`🎬 Videos · ${displayMaestroNombre(videoModalMaestro)}`:""}>
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#4BBCE015,#5B2D8E15)",border:"1.5px solid #4BBCE055",borderRadius:14,padding:"12px 14px",marginBottom:16,fontSize:13,color:"#2D1B4E"}}>
        ...
        <div style={{color:"#5B2D8E",fontSize:12}}>Cada maestro debe grabar un video de su clase para presentarlo el próximo domingo. Aquí puedes calificar si lo hizo, si quedó bien y si lo envió a tiempo. La puntuación <strong>afecta el promedio de evaluación</strong> del maestro (20% del total).</div>
      </div>
      ...
      {sesionesFiltradas.length===0&&(
        <div style={{textAlign:"center",padding:"40px 20px",color:"#7B6B9A",fontSize:14}}>
          No hay sesiones programadas para {selClase} aún.
        </div>
      )}
          ...
          <div key={ses.id} style={{...S.card,borderLeft:`5px solid ${v?scoreColor:"#DDD0F0"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"#7B6B9A",marginTop:2}}>📅 {ses.fecha} · 🎓 {displayMaestroNombre(ses.maestro)}</div>
              </div>
          ...
          <div>
            <div style={{background:"#F5F0FF",borderRadius:12,padding:"12px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
              ...
              <div>
                ...
                <div style={{fontSize:12,color:"#7B6B9A",marginTop:2}}>📅 {editVideo.fecha} · 🎓 {displayMaestroNombre(editVideo.maestro)}</div>
              </div>
            </div>
            ...
            <label style={S.label}>¿El maestro envió el video?</label>
            <div style={{display:"flex",gap:10,marginBottom:16}}>
              {[[true,"✅ Sí, lo envió","#4CAF50"],[false,"❌ No lo envió","#EF5350"]].map(([val,lbl,col])=>(
                <button key={String(val)} onClick={()=>setEditVideo(v=>({...v,hizo:val}))}
            ...
            {editVideo.hizo&&(
              <>
                {/* Calidad del video */}
                <label style={S.label}>⭐ Calidad del video (1–5)</label>
                <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:16}}>
                  {[1,2,3,4,5].map(s=>(
                    <button key={s} onClick={()=>setEditVideo(v=>({...v,calidad:s}))}
                      style={{width:46,height:46,borderRadius:12,border:`2px solid ${editVideo.calidad===s?"#F5C842":"#DDD0F0"}`,background:editVideo.calidad===s?"#5B2D8E":"#F5F0FF",color:editVideo.calidad===s?"#F5C842":"#7B6B9A",fontWeight:900,cursor:"pointer",fontSize:22,fontFamily:"inherit"}}>★</button>
                ...
                {/* ¿A tiempo? */}
                <label style={S.label}>⏰ ¿Lo envió a tiempo?</label>
                <div style={{display:"flex",gap:10,marginBottom:20}}>
                  {[[true,"⏰ Sí, a tiempo","#4CAF50"],[false,"⏱ No, tarde","#FF7043"]].map(([val,lbl,col])=>(
                    <button key={String(val)} onClick={()=>setEditVideo(v=>({...v,aTiempo:val}))}
            ...
            <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:14}} onClick={saveVideo}>💾 Guardar Calificación</button>
          </div>
        )}
      </Modal>
    </div>
          ...
          {eventos.filter(e=>e.tipo===tipo).map((e)=>{
            ...
            return(
              <div key={e.id} style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:12,paddingBottom:12,borderBottom:"1px solid #DDD0F0",opacity:cumplido?0.7:1}}>
                ...
                {!readOnly&&(
                  <button style={{...S.btn("#4BBCE0"),padding:"5px 9px",fontSize:12}} onClick={()=>openEdit(e)}>✏️</button>
                )}
              </div>
            );
          })}
...
// ── Sub-componente: badges de merienda/taller por sesión ──
  ...
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:6}}>
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {m?.merienda && <span style={{...S.badge("#4BBCE0"),fontSize:11}}>🍎 Merienda{m.meriendaCosto?" · €"+m.meriendaCosto:""}{merDonativo?" 🎁 donativo":""}</span>}
        {m?.trabajoManual && <span style={{...S.badge("#E84F9B"),fontSize:11}}>✂️ T.Manual{m.trabajoManualCosto?" · €"+m.trabajoManualCosto:""}{tmDonativo?" 🎁 donativo":""}</span>}
      ...
      {prevObs.map((obs,i) => (
        <div key={obs.id||i} style={{marginBottom:i<prevObs.length-1?10:0,paddingBottom:i<prevObs.length-1?10:0,borderBottom:i<prevObs.length-1?"1px dashed #F5C84255":"none"}}>
          <div style={{display:"flex",gap:6,marginBottom:3,flexWrap:"wrap"}}>
            <span style={{fontSize:11,fontWeight:800,color:"#5B2D8E",background:"#5B2D8E15",borderRadius:6,padding:"2px 6px"}}>{obs.leccion||"Sesión"}</span>
            <span style={{fontSize:11,color:"#7B6B9A"}}>📅 {formatFecha(obs.fecha)}</span>
            {obs.quienObservacion&&<span style={{fontSize:11,color:"#7B6B9A"}}>· por {displayMaestroNombre(obs.quienObservacion)}</span>}
          </div>
...
function TeacherCalif({user,data,onUpdateCalif,onUpdateMerienda,superMaestro=false,activeClase=null}){
  ...
  const resetCalifSesion=(ses)=>{
    if(!confirmDelete("¿Resetear todas las calificaciones de esta sesión? Los alumnos quedarán como ausentes y podrás volver a calificar."))return;
  ...
  const borrarCalifAlumno=()=>{
    ...
    if(!confirmDelete("¿Borrar la calificación de "+shortDisplayName(selNino.nombre)+" en esta sesión? Quedará como ausente."))return;
  ...
  return(
    <div style={{padding:"1rem 1rem 6.25rem"}}>
      ...
      {misSesiones.map(ses=>(
        <div key={ses.id} style={{...S.card,borderLeft:`5px solid ${CLASE_COLORS[ses.grupo]||"#5B2D8E"}`}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
            <div style={{fontSize:11,fontWeight:700,color:"#7B6B9A",letterSpacing:0.5}}>ASISTENCIA AUTOMÁTICA</div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              {calificaciones.some(c=>c.clase===miClase&&c.sesionId===ses.id)&&(
                <button style={{...S.btn("#FFF0E6","#C62828"),padding:"5px 10px",fontSize:11}} onClick={()=>resetCalifSesion(ses)} title="Borrar todas las calificaciones de esta sesión para poder volver a calificar">🔄 Resetear</button>
              )}
            </div>
          </div>
          {misNinos.map(n=>{
            ...
            return(
              <div key={n.nombre||n.id} style={{display:"flex",alignItems:"center",gap:10,paddingBottom:10,marginBottom:10,borderBottom:"1px solid #DDD0F0",opacity:asistio?1:0.55}}>
                ...
                <button style={{...S.btn(calif?"#4BBCE0":"#5B2D8E","#FFFFFF"),padding:"8px 13px",fontSize:calif?13:18,fontWeight:900,borderRadius:12}} onClick={()=>open(n,ses)}>{calif?"✏️":"+"}</button>
              </div>
            );
          })}
        </div>
      ))}
      <Modal open={sesModal} onClose={()=>setSesModal(false)} title="📋 Datos de la Sesión">
        ...
        <div style={{marginBottom:16}}>
          <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"12px 14px",background:"#F5F0FF",borderRadius:12,marginBottom:10}}>
            ...
            <div><div style={{fontWeight:700,fontSize:14}}>🍎 ¿Llevó merienda?</div></div>
          </label>
          {sesData.merienda&&(
            <div style={{marginBottom:10}}>
              ...
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginTop:8,fontSize:13,color:"#5B2D8E"}}>
                ...
                <span>🎁 Marcar como donativo</span>
              </label>
            </div>
          )}
          <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"12px 14px",background:"#FFF0F8",borderRadius:12,marginBottom:10}}>
            ...
            <div><div style={{fontWeight:700,fontSize:14}}>✂️ ¿Usó trabajo manual?</div></div>
          </label>
          ...
          {(sesData.merienda||sesData.trabajoManual)&&(
            <div style={{background:"linear-gradient(135deg,#4CAF5020,#4CAF5035)",border:"2px solid #4CAF5055",borderRadius:14,padding:"14px",marginTop:10,textAlign:"center"}}>
              <div style={{fontSize:12,color:"#7B6B9A",marginBottom:4}}>GASTO TOTAL DE LA SESIÓN</div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={`Calificar: ${selNino?shortDisplayName(selNino.nombre):""}`}>
        ...
        {form.quienCalifico&&(
          <div style={{fontSize:11,color:"#7B6B9A",marginBottom:12}}>Calificado por: <strong>{displayMaestroNombre(form.quienCalifico)}</strong> · {formatCalifFecha(form.fechaCalificacion)}</div>
        )}
        <button style={{...S.btn("#5B2D8E","#FFFFFF",true),padding:12,marginBottom:16}} onClick={saveSoloCalificacion}>💾 Guardar calificación</button>
        ...
        <button style={{...S.btn("#4BBCE0","#FFFFFF",true),padding:12,marginBottom:16}} onClick={saveSoloObservacion}>💾 Guardar observación</button>

        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          {selNino&&selSes&&calificaciones.some(c=>c.alumno===selNino.nombre&&c.clase===miClase&&c.sesionId===selSes.id)&&(
            <button style={{...S.btn("#FFF0F0","#EF5350"),padding:14,border:"1.5px solid #EF535044"}} onClick={borrarCalifAlumno}>🗑 Borrar calificación</button>
          )}
        </div>
      </Modal>
    </div>
