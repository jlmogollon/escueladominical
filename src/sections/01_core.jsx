...
function flipName(str) {
  ...
  const primerNombres = ["josÃ©","jose","juan","marÃ­a","maria","carlos","pedro","ana","diana","luz","martha","mabel","deisy","blanca","verÃ³nica","veronica","andrÃ©s","andres","alejandro","samuel","valeria","cindy","astrid","lucy","michael","rosy","Ã¡ngel","angelo","david","daniel","javier","josÃ© luÃ­s","jose luis"];
...
function confirmDelete(msg="Â¿EstÃ¡s seguro de que deseas eliminar este registro? Esta acciÃ³n no se puede deshacer.") {
  return window.confirm(msg);
}
// Compress image to max 128x128px, quality 0.5, returns base64 data URI (mÃ¡s pequeÃ±o para no superar 1 MB por doc en Firestore)
...
// MODELO DE DATOS: La lista de ALUMNOS es la Ãºnica fuente de verdad.
// De ella se derivan:
//   - CLASES: agrupaciÃ³n por clase (CORDERITOS, VENCEDORES, CONQUISTADORES, ADOLESCENTES).
//   - FAMILIAS: agrupaciÃ³n por familia (padres, telÃ©fonos, etc.).
// Las pantallas Clases y Familias muestran datos derivados; la ediciÃ³n de niÃ±os
// se hace solo en la pestaÃ±a Alumnos. No se debe usar clases ni familias para
...
// Estructura: primer nombre, [segundo nombre], primer apellido, [segundo apellido]. Segundo nombre y segundo apellido pueden estar vacÃ­os.
...
// Normaliza nombres/apellidos para comparaciones (minÃºsculas, sin tildes)
function normalizeNamePart(s){
  return (s||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"");
}

// Compara dos nombres de maestro ignorando orden/apariencia.
// Estrategia: normalizar a tokens (sin tildes, en minÃºsculas) y considerar que
...
const CRITERIOS = [
  ...
  {logro:"CONOCIMIENTO",key:"conocimiento",item:"Historias bÃ­blicas"},
  {logro:"ORACIÃ“N",key:"oracion",item:"HÃ¡bito de oraciÃ³n"},
  {logro:"SERVICIO",key:"servicio",item:"Compartir y ayudar"},
  {logro:"RESPETO",key:"respeto",item:"Respeto hacia los demÃ¡s"},
  {logro:"PARTICIPACIÃ“N",key:"participacion",item:"Actividades grupales"},
...
const DEFAULT_FINANZAS = { gastos:[], actividades:[], donativos:[], ofrendaNinos:[] };

// Maestros autorizados exclusivamente para la clase ADOLESCENTES (sin auxiliar): JosÃ© MogollÃ³n, Yuliet Ceballos, Martha Caro, Ana MarÃ­a HernÃ¡ndez, JosÃ© HernÃ¡n DÃ­az
const ADOLESCENTES_MAESTROS = [
  "MogollÃ³n JosÃ© Luis",
  "Ceballos Yuliet",
  "Caro Martha",
  "HernÃ¡ndez Ana MarÃ­a",
  "DÃ­az JosÃ© HernÃ¡n",
];

// Helpers para config dinÃ¡mica de clases
...
function deriveClases(alumnos,clasesConfig){
  ...
  (alumnos||[]).forEach(a=>{
    ...
    out[key].push({ id: a.id, nombre: a.nombre, primerNombre: a.primerNombre, segundoNombre: a.segundoNombre, primerApellido: a.primerApellido, segundoApellido: a.segundoApellido, edad, foto: a.foto||null, nacimiento: a.nacimiento, cumpleanos, bautizado: !!a.bautizado, sellado: !!a.sellado, padre: a.padre||"", madre: a.madre||"", telPadre: a.telPadre||"", telMadre: a.telMadre||"" });
  });
  return out;
}
// Devuelve {nombre, clase} de quienes aparecen en calificaciones pero no en alumnos (evita pÃ©rdida de lista).
...
// Devuelve {nombre, clase, foto} de quienes aparecen en el objeto clases guardado pero no en alumnos (foto por si el niÃ±o en clases la tiene).
...
function restoreFotosFromClases(alumnos,loadedClases){
  ...
  const niÃ±osConFoto=[]; // { nombre, clase, foto }
  Object.entries(loadedClases).forEach(([claseKey,arr])=>{
    if(!Array.isArray(arr))return;
    const clase=normalizarClase(claseKey);
    arr.forEach(n=>{
      ...
      niÃ±osConFoto.push({nombre,clase,foto:n.foto});
    });
  });
  let changed=false;
  alumnos.forEach(a=>{
    ...
    const match=niÃ±osConFoto.find(n=>samePersonName(n.nombre,a.nombre)&&normalizarClase(n.clase)===claseA);
...
// Restaura todos los datos de alumnos (bautizado, sellado, padres, telÃ©fonos, nacimiento, familia, nombre desglosado) desde clases o familias guardados.
// Busca en todas las claves del objeto clases por si el formato de clave difiere; prioriza datos no vacÃ­os del almacenamiento.
...
// No crea alumnos nuevos: solo apunta las calificaciones al alumno Ãºnico que ya existe (y normaliza la clase).
...
function deriveFamilias(alumnos){
  ...
  const getApellidos=(a)=>{
    if(a.familia&&norm(a.familia))return norm(a.familia);
    if(a.primerApellido!=null||a.segundoApellido!=null)return norm(buildFamiliaAlumno(a.primerApellido||"",a.segundoApellido||""));
    const p4=parseNombre4(a.nombre||"");return norm(buildFamiliaAlumno(p4.primerApellido,p4.segundoApellido));
  };
  const groups={};
  (alumnos||[]).forEach(a=>{
    const padre=norm(a.padre);
    const madre=norm(a.madre);
    const tienePadreOMadre=!!padre||!!madre;
    const apellidos=getApellidos(a);
    let key;
    if(!tienePadreOMadre)key="__solo__"+a.id;
    else{key=padre+"|"+madre+"|"+apellidos;if(!key||key==="||")key="__solo__"+a.id;}
  ...
  Object.entries(groups).forEach(([key,members])=>{
    // No conformar familia si no hay al menos un padre o madre; alumno solo no se considera familia
    if(key.startsWith("__solo__"))return;
    numCounter++;
    const first=members[0];
    const familiaLabel=norm(first.familia)||getApellidos(first)||displayNameAlumno(first)||first.nombre||"";
    const familyKey=norm(first.padre)+"|"+norm(first.madre)+"|"+getApellidos(first);
    members.forEach(a=>{
      let edad=null,cumpleanos=null;
      if(a.nacimiento){
        try{
      ...
      derived.push({ id: a.id, num: numCounter, familyKey, familia: familiaLabel, padre: a.padre||"", madre: a.madre||"", telPadre: a.telPadre||"", telMadre: a.telMadre||"", alumno: a.nombre, primerNombre: a.primerNombre, segundoNombre: a.segundoNombre, primerApellido: a.primerApellido, segundoApellido: a.segundoApellido, edad, cumpleanos, nacimiento: a.nacimiento||"", clase: normalizarClase(a.clase), bautizado: !!a.bautizado, sellado: !!a.sellado, foto: a.foto||null });
    });
  });
  return derived;
}
...
const EVAL_LABELS = ["Dominio BÃ­blico","Habilidad EnseÃ±ar","Claridad","Recursos Visuales","Manejo Grupo","EmpatÃ­a","Creatividad","PreparaciÃ³n","Compromiso","AplicaciÃ³n PrÃ¡ctica","âœ… Cumplimiento de Clases"];
